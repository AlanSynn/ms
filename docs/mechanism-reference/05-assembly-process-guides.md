...
Expected export contents (generated by `components/stages/assembly/AssemblyGuide.tsx`):

```text
<project-id>-assembly.pdf
<project-id>-character-sheet.pdf
```
...
3. **Add output G3 at H9**
   ...
   - Check: the two G3 pitch circles touch without overlap.
4. **Check gear mesh**
   - Use a real drive gear attachment hole.
   - Check: turn the drive gear and confirm the output gear counter-rotates on its fixed axle.

Gear train final check:

- Distance `H6-H9` is 3 cells / 60 mm.
- Both endpoint centres are board-fixed.
- Meshing gears touch lightly, not forcefully.
...
3. **Plug in guide cartridge**
   - Mount the U-channel guide cartridge above the cam on board holes J11/J9.
