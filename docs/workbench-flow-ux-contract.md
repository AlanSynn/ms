...
- `utils/mechanismSceneContract.ts` and `utils/assemblySceneFrame.ts` are harnessable plain-data seams. Assembly consumes `AssemblySceneFrame` through the shared Foundry Three renderer.
...
| Design SSOT | Design still uses the integrated `ThreePuppetPreview` automata path and has not fully consumed `MechanismSceneContract` for layer/z/pin parity. | `DesignFoundryPreview.tsx`, `utils/designAutomataProjection.ts`, `docs/prd/foundry-assembly-ssot-plan.md` | Keep current preview until replaced, but block new private mechanism stack/z logic in Design. |
