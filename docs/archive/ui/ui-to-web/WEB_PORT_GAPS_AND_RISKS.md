...
1. **Coordinate duplication across tabs**
   Current Qt use multiple view/scene surfaces. Web must keep canonical mm coords in one scene store.

2. **Parametric editing display/state drift**
   Handles computed from current mechanism params on render. Drag update same canonical params, then re-render. No second hidden copy of handle positions.
...
4. **Blueprint/assembly guide state loss**
   Export read actual `MechanismInstance` transform/board/character state. No generic/default board placement if real state exists.
...
6. **Body part/skeleton editing**
   Adding/removing skeleton points and body part layers must update project state, character rendering, path binding, mechanism binding, and blueprint export refs.
...
| Partial 4-bar valid range | UI show valid angle interval, allow animation within interval |
| Duplicate same-type mechanisms | two 4-bars stay two distinct instances in design + blueprint |
| Foundry export position | Add to Mechanism Tab preserve board coords + output point |
...
| Blueprint export state | assembly guide use actual scene placement |