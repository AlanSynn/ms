...
| --- | --- | --- |
| File menu | New | clear/create project, dirty-state guard |
| File menu | Load Project... | file picker/import project |
| File menu | Recover Browser Autosave... | open autosave recovery flow |
| File menu | Download Snapshot | download current local project snapshot |
...
| Help menu | Keyboard Shortcuts, About... | generated shortcut reference + About modal |
...
| --- | --- |
| File → Exit | Browsers can't reliably close app window; no fake "not available" command. |
| Help → Check for Updates... | No updater contract in browser build; keep absent until native/Tauri updater exists. |
| Character → Choose Save Folder… | Browser downloads go to browser-configured download location; selecting dir wouldn't make existing exports write there. |
...
| 2 Mechanism Generation | Parametric Edit | toggles handle layer + param edit mode |
...
| Controls | Parameter sliders | update mechanism params, snap if enabled |
