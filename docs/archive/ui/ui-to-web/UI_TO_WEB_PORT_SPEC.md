...
- Central widget: vertical layout, single `QTabWidget`
...
Experiment mode title prefix (`1.`, `2.`, `3.`); Foundry hidden in experiment mode.
...
- Toolbar: optional compact quick-action row, hidden by default (match Qt).
...
`Choose Save Folder…` retired from browser UI: exports use normal browser downloads, so directory picker misleading until native/Tauri writer owns output path.
...
- Path data stored per part in project state, not canvas node only.
- Smoothness deterministic; update visual path + stored params from same source.
...
| Group | Controls | Notes |
| --- | --- | --- |
| `1 Parts for Mechanisms` | `mechanism_layers_list` | black/gray = path availability; selects mechanism target layer/part |
...
| `4 Blueprint Export` | `Export Blueprint` + explanatory label | exists in `mechanism_design_tab_layout.py`; shown in current app |
| `5 View Controls` | `+`, `−`, `⌖`, `⎈` | zoom/fit/center character |

### Current complexity hotspots

Important for web rebuild: user reported mismatches here.

| Concern | Current implementation area | Web requirement |
| --- | --- | --- |
| Mechanism layer data | `mechanism_layers: dict[str, dict]` across tab/services | make `MechanismInstance` typed, globally stored |
...
- Specialized editor mode. Modal with embedded mini canvas, or better: persistent canvas + modal side panel + temporary layer preset.
- Must support add/remove skeleton points + part layers (user-requested).
...
Dropped from web port. Character import uses bundled starters, project/package import, or local image upload with Web ONNX.
...
2. Implement persistent canvas with grid/sheet + viewport controls.
...
- [ ] Character scale + position identical in Character, Editor, Mechanism, Blueprint.
- [ ] 2 cm grid identical in every canvas mode.
- [ ] Parametric handle position = numeric parameter value after every drag.
- [ ] Mechanism drag updates stored parameters + immediate visuals from same transaction.
- [ ] Mechanism import from Foundry preserves position, board context, type, params, output point, valid angle range.
- [ ] Multiple mechanisms of same type stored/rendered/exported as separate instances.
- [ ] Part layers + skeleton points add/remove, reflected in all tabs.
