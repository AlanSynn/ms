...
## Reference contract
- MIT CDMC: characters = rigid components connected by pin, point-on-line, phase/gear, fixed-state constraints. Runtime maps MotionSmith mechanisms to constraints, solves/updates from one input driver phase.
- CDMC gear phase constraint: `Cc = αi - f(αj)`, spur/bevel gears use `f(α) = -r * α` where `r` = tooth-count ratio. Planar external gear train: every mesh flips direction, endpoint speed ratio set by first + last pitch radii.
- CDMC connects driving mechanisms back to one input driver via intermediate gear trains. MotionSmith keeps as `driverGroupId + driverPhaseOffset` first, then adds full constraint graph solver once multi-driver UI ready.
...
- `driverGroupId`: same-group mechanisms driven by one virtual input shaft.
...
- `speed2` + `phase` remain explicit until full driver graph replaces them.
...
- Fabrication inserts idler gear layers for trains > 2 gears, scales required parts with gear count.
...
- `driverPhaseOffset` shifts mechanism local input phase before kinematic constraints sampled.
- `phase` remains secondary-output phase for legacy 5-bar/geared mechanisms.
...
## Verification rules
- Tests must assert physical distances/ratios, not just visibility.
