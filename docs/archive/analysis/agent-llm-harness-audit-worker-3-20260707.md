...
- Deprecated/low-priority skills: `ecomode, swarm, deepsearch, tdd, build-fix, security-review, visual-verdict, web-clone, frontend-ui-ux, review, ask-claude, ask-gemini, help, note, trace, ralph-init`.
...
Needed now:
- `analyze`, `plan`, `ralplan`, `deep-interview`, `code-review`, `hud`, `doctor` and engineering agents (`explore`, `executor`, `debugger`, `dependency-expert`, `test-engineer`, `verifier`).
- Plugin `browser`.
...
## Risk note
Subagent fan-out not executed — no observable native subagent invocation facility in this worker loop.