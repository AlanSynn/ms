...
- `analyze`, `plan`, `ralplan`, `deep-interview`, `code-review`, `design` (UI/scene-heavy fixes), `explore`/`dependency-expert`/`test-engineer`/`verifier` agents, `hud`, `doctor`.
- Plugin `browser` for local app interaction/testing.

Non-essential here (keep disabled unless task needs them):
...
Lowest-risk reduce context/noise (lowest impact):
1. Disable deprecated workflow skills: `deepsearch`, `ecomode`, `tdd`, `build-fix`, `visual-verdict`, `frontend-ui-ux`, `swarm`, `review`, `ask-claude`, `ask-gemini`, `help`, `note`, `trace`, `web-clone`.
2. Disable artifact plugins unless doc/sheet/PDF/presentation work active in branch: `documents`, `pdf`, `spreadsheets`, `presentations`, `template-creator`.
3. Keep `autopilot`, `plan/ralplan`, `deep-interview`, `code-review`, `prometheus-strict*`, `agent` roles (`explore`, `executor`, `debugger`, `verifier`) enabled for repo workflow.

## 4) Non-repo context injection check

Checked context injection path in team/runtime snapshots:
...
- All referenced files resolve under repo path `/Users/alansynn/Documents/MechAnim/.omx/context/*`.
- No `./.omx/context/state` path found under repo or `/Users/alansynn/Documents/MechAnim/.omx/context`.

Conclusion: no non-repo context injection observed.