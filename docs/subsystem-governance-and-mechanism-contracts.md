...
- Current risk: `components/stages/foundry/ThreeFoundryPreview.tsx` still contains renderer branches; `utils/mechanismPreviewStacks.ts` now centralizes pin/z semantics but still has type branches until compiler contracts emit those facts. Those branches are temporary debt unless they only project already-derived shared contracts into DOM/Three objects.
