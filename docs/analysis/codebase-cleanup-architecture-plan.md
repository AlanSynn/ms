...
- Local ignored junk can be removed when seen: `.DS_Store`, `resources/.DS_Store`, `resources/examples/.DS_Store`, `fabrication/__pycache__/`, `test-results/`. Do not delete `.agents/`, `.omx/`, `docs/archive/ports/to-port-web-onnx/`, `dist/`, or `node_modules/` as cleanup.
...
| `utils/mechanismInventory.ts`                                |    62 | Done: Foundry/Design rendered inventory counts use the graph compiler recipe directly. Keep this pure, kit-aware, and shared by Three preview probes instead of stage-local wrappers.                                                                                                                                                 |
| `utils/mechanismPreviewStacks.ts`                             |   543 | Done: Shared mechanism pin-stack/z-order helper lives outside stage folders. Keep it pure and shared so Foundry/Design/Assembly z-stack semantics do not drift.                                                                                                                                                                                  |
...
2. **Stage components**
   ...
   - Done: `components/stages/foundry/MechanismFoundry.tsx`, `FoundryCanvasPane.tsx`, `FoundryCanvasChrome.tsx`, `FoundryOverlayLayer.tsx`, `FoundryWorkflowPanel.tsx`, `FoundryInspectorPanel.tsx`, `MechanismLinkagePreview.tsx`, `mechanismLinkagePreviewHelpers.ts`, `foundryPreviewGeometry.ts`, `ThreeFoundryPreview.tsx`, `foundryThreePrimitives.ts`, `foundryThreeRenderLayers.ts`, and `mechanismPreviewStacks.ts` own the Foundry stage/renderer seams outside `App.tsx`; Foundry and Mechanism Design still mount the same Three preview component and share pin-stack/z-order contracts.
