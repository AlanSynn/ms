...
| `utils/fabrication.ts`                                        |   794 | Split validation/export and package generation next only with stronger gates. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, readiness sampling/tolerance math lives in `utils/fabricationReadiness.ts`, render-plan validation/z-order lives in `utils/fabricationRenderPlan.ts`, Blueprint SVG rendering lives in `utils/fabricationBlueprintSvg.ts`, planetary/linkage sizing lives in `utils/fabricationSizing.ts`, import-free PDF document primitives live in `utils/simplePdf.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
