...
| `components/stages/assembly/assemblyGeometry.ts`              |    97 | Legacy helper kept only while referenced by non-deleted paths; do not reintroduce SVG workbench authority. Prefer `utils/assemblySceneFrame.ts` for new Assembly build facts. |
