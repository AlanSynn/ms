...
- Chunk budget: `vite.config.ts` keeps `chunkSizeWarningLimit: 2400` for the lazy Rapier browser chunk. This is not permission for growth.

## Image recognition removed — 2026-08-20

- The classroom build contains no ONNX model, ONNX Runtime dependency, inference/cache worker, image-to-rig UI, or recognition starter image.
- `scripts/check-no-image-recognition.mjs` blocks both source regressions and forbidden production output. Web, Tauri, CI, and tag-only Pages builds run this guard.
- Character packages, full-project import, ordinary scene-object images, and GIF/video Trace remain supported. Legacy `onnx-mask` provenance is read-only project compatibility, not a runtime feature.
...
| `hooks/useAppCharacterImportActions.ts`                      |   164 | Done: local character package/project import, pending review, skeleton export, and Getting Started entry actions live outside App. Keep it intake/review-only; no image recognition, mechanism fitting, canvas rendering, or stage UI. |
...
| `components/stages/character/ProgressBlock.tsx`               |    78 | Done: local character-package import progress UI lives outside the app shell. Keep it presentation-only.                                                                                                                                                                             |
...
| `components/stages/character/CharacterImportOverlays.tsx`     |   161 | Done: character import status/review overlays live outside the app shell. Keep it presentation-only; package acceptance remains App/ProjectState-owned.                                                                                                                                                          |
...
| `components/stages/path/PathEditor.tsx`                       |   376 | Done: Path stage wrapper lives outside the app shell. Keep it orchestration-only; the shared Three canvas, workflow, and inspector panes own view code while motion/coordinate math stays in utils.                                                                                                                            |
| `components/stages/path/SceneSketch.tsx`                      |     0 | Deleted 2026-08-16: Path no longer mounts an SVG authoring surface; the shared Three scene owns front-view drawing, path handles, and 3D inspection.                                                                                                                                                                           |
| `components/stages/path/PartShape.tsx`                        |     0 | Deleted 2026-08-16: Path no longer needs an SVG-only part renderer; solid component geometry comes from ThreePuppetPreview.                                                                                                                                                                                              |
| `components/stages/path/MechanismRecommendationSheet.tsx`     |   230 | Done: Path recommendation modal lives outside the app shell and previews board-fit overlays. Keep it UI-only; scoring and fabrication fitting stay in utils/mechanismRecommendations.                                                                                                                                                 |
...
| `utils/mechanismRecommendations.ts`                           |   637 | Done: pure recommendation/fitting seam shared by Foundry export and recommendation flows. Keep deterministic; no DOM/storage side effects.                                                                                                                                                                                            |
...
| `components/stages/foundry/ThreeFoundryPreview.tsx`           |   962 | Done: shared Foundry/Design/Assembly Three renderer seam delegates browser telemetry, primitive mesh/material builders, dynamic render-layer dispatch, and tab-specific viewer contracts outside the render body. Keep it behavior-identical; split camera/host lifecycle only behind contract/browser evidence.                                                              |
...
| `components/stages/blueprint/BlueprintControlPanel.tsx`      |   269 | Done: Blueprint left workflow controls, package generation, download buttons, character sheet downloads, and recipe list live outside the stage wrapper. Keep it presentation/action-only; preview SVG and export package rules stay in BlueprintExport/utils. |
...
| `components/stages/options/Options.tsx`                       |   350 | Done: Options stage wrapper lives outside the app shell and owns one full-center, horizontally arranged settings surface while delegating shared field wrappers. Keep it settings orchestration-only; ProjectState actions own mutation and persistence/export rules stay elsewhere. |
...
| `components/stages/assembly/AssemblyControlPanel.tsx`         |   192 | Done: Assembly left workflow controls, lane/mode switches, recipe picker, character sheet download, and step list live outside the stage wrapper. Keep it presentation/action-only; playback state and recipe derivation stay in AssemblyGuide. |
| `components/stages/assembly/AssemblyCanvasPane.tsx`           |    61 | Done: Assembly center canvas chooser owns only the mechanism/character Three preview mount, `AssemblySceneFrame` strip, and empty state. Keep scene derivation in shared utils and drawing in Three preview leaves. |
...
| `components/stages/assembly/AssemblySceneFrame.tsx`           |    99 | Done: read-only Assembly step strip renders canonical frame facts from pure builders. Keep it presentation-only; no SVG workbench authority, no ProjectState mutation, no renderer geometry. |
| `utils/assemblySceneFrame.ts`                                  |   155 | Done: DOM-free Assembly frame builder owns canonical phase/motion/coord facts for mechanism and character steps. Keep deterministic and import-light; Three/renderers consume it rather than duplicating assembly grammar. |
| `utils/mechanismSceneContract.ts`                              |    92 | Done: DOM-free mechanism visual contract bridges fabrication render plans, readiness, and stack metadata for Assembly/Design. Keep Foundry/fabrication as source of truth. |
| `components/stages/assembly/assemblyGeometry.ts`              |    0 | Deleted 2026-08-31: the contract test was the only consumer; board/canvas projector, coordinate, and smoothing helpers were deleted with it. Do not revive; Assembly build facts live in `utils/assemblySceneFrame.ts`. |
...
| `components/stages/mechanism/DesignFoundryPreview.tsx`        |   362 | Done: Design's Foundry preview adapter lives outside App and keeps `ThreeFoundryPreview` as the single mechanism renderer shared with Foundry while `ThreePuppetPreview` supplies character/object context only. Keep it adapter-only; renderer rules stay in Foundry/physics/fabrication helpers.                                                                                                      |
| `utils/foundryPreviewModel.ts`                                |    75 | Done: shared Foundry-style live preview model derives playback frame, physical simulation, point traces, path overlays, and physics overlay data for non-Foundry consumers. Keep DOM-free and consume Foundry/fabrication helpers rather than stage-local mechanism branches. |
...
| `utils/fabrication.ts`                                        |   309 | Split validation and package assembly next only with stronger gates. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, readiness sampling/tolerance math lives in `utils/fabricationReadiness.ts`, render-plan validation/z-order lives in `utils/fabricationRenderPlan.ts`, package recipe derivation lives in `utils/fabricationRecipes.ts`, character SVG/PDF/STL artifacts live in `utils/fabricationCustomParts.ts`, assembly guide HTML/PDF artifacts live in `utils/fabricationAssemblyGuide.ts`, cut-sheet PDF output lives in `utils/fabricationCutSheetPdf.ts`, Blueprint SVG rendering lives in `utils/fabricationBlueprintSvg.ts`, planetary/linkage sizing lives in `utils/fabricationSizing.ts`, import-free PDF document primitives live in `utils/simplePdf.ts`, character cut-sheet layout lives in `utils/fabricationCharacterPrintLayout.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
...
| `utils/simplePdf.ts`                                          |    52 | Done: import-free PDF document primitives, bounded multi-page PDF assembly, PDF text escaping, numeric formatting, circle path, color conversion, and simple single-page PDF helper live outside the broad fabrication runtime. Keep domain-free: no ProjectState, fabrication recipes, package generation, DOM, React, Three, or renderer ownership. |
...
| `utils/fabricationCustomParts.ts`                             |   151 | Done: bounded 1–2 page character custom-parts SVG/PDF plus STL artifact generation live outside the broad fabrication runtime. Keep it artifact-only: no package generation, validation, mechanism recipes, DOM, React, Three, or renderer ownership. |
| `utils/fabricationAssemblyGuide.ts`                           |   114 | Done: assembly guide exploded SVG, printable HTML, and PDF artifact generation live outside the broad fabrication runtime. Keep it artifact-only: no package generation, validation, cut-sheet/custom-parts output, DOM, React, Three, or renderer ownership. |
| `utils/fabricationCutSheetPdf.ts`                             |    48 | Done: cut-sheet PDF artifact generation lives outside the broad fabrication runtime. Keep it cut-sheet-only: no package generation, validation, assembly/custom-parts output, DOM, React, Three, or renderer ownership. |
| `components/Canvas.tsx`                                       |     0 | Deleted 2026-07-04: runtime had no importers and tests/docs were the only owners. Do not revive; active views use `ThreePuppetPreview`, shared `ThreeFoundryPreview`, and export-only blueprint SVG renderers.                                                                                                                     |
| `components/TrackingModal.tsx`                                |   930 | Keep GIF/video Trace bounded and worker-owned; it is independent of removed image recognition.                                                                                                                                                                                                                                                                    |
...
bun run build
GIT_OPTIONAL_LOCKS=0 git -c core.fsmonitor=false diff --check
env -u NO_COLOR PLAYWRIGHT_SERVER=preview PLAYWRIGHT_WORKERS=2 bunx playwright test tests/browser/workflow.spec.ts -g "Character tab processing|School build has no image-recognition|Load package review|Replacement package|Every top menu command|guided classroom lesson" --workers=2
```

Use this gate when moving character intake/review actions. It proves package/project import, replacement review, command wiring, guided lesson startup, and image-recognition absence through real browser workflows.
...
1. **Command and app shell seams**
   ...
   - Done: `hooks/useAppCharacterImportActions.ts` owns local character-package/project import, pending/replacement review, skeleton export, and Getting Started entry actions outside `App.tsx`; it stays intake/review-only and does not own image recognition, mechanism fitting, or UI rendering.
...
2. **Stage components**
   ...
   - Done: legacy Assembly SVG workbenches were deleted; `components/stages/assembly/AssemblySceneFrame.tsx` renders a read-only callout strip from `utils/assemblySceneFrame.ts`, while `AssemblyThreePreview.tsx` remains the visual scene surface.
   ...
   - Done: `components/stages/path/PathEditor.tsx`, `PathWorkflowPanel.tsx`, `PathCanvasPane.tsx`, `PathInspectorPanel.tsx`, and `MechanismRecommendationSheet.tsx` own the Path Editor UI seam outside `App.tsx`; Path uses the shared Three front/iso scene for viewing, drawing, and point editing, still receives ProjectState/actions, and does not own mechanism rules. `components/stages/mechanism/MechanismParametricEditor.tsx` owns the shared Foundry/Design compact gear/link/idler/cam parameter UI outside `App.tsx`, and `mechanismParamPolicy.ts` owns deterministic legacy numeric parameter visibility/clamping.
   ...
   - Done: `components/stages/options/Options.tsx` owns the Options stage wrapper and full-center settings surface outside `App.tsx`, while `OptionsSettingsControls.tsx` owns section/field wrappers; the former decorative preview is deleted, and Options still receives ProjectState/actions without owning persistence or export rules.
   - Done: `components/stages/assembly/AssemblyGuide.tsx`, `AssemblyControlPanel.tsx`, `AssemblyCanvasPane.tsx`, `AssemblySceneFrame.tsx`, `AssemblyThreePreview.tsx`, and `AssemblyInspectorPanel.tsx` own the Assembly Guide stage wrapper plus workflow/canvas/scene-strip/inspector leaves outside `App.tsx`; Assembly still receives ProjectState/actions and shell playback state while recipe/stack/frame derivation stays in shared utils.
...
3. **Domain helpers**
   ...
   - Assembly playback derivation remains in `utils/assemblyPlayback.ts`; canonical frame facts live in `utils/assemblySceneFrame.ts`.
...
5. **Delete legacy**
   - Runtime-unused `components/Controls.tsx`, `utils/zStack.ts`, and legacy Assembly SVG workbenches are gone and locked by contract tests.
...
- Extracted assembly playback derivation to `utils/assemblyPlayback.ts`; later retired the SVG workbench split in favor of `utils/assemblySceneFrame.ts`, `utils/mechanismSceneContract.ts`, and `components/stages/assembly/AssemblySceneFrame.tsx` as the harnessable Three-first Assembly scene contract.
