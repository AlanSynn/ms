...
| `components/stages/options/Options.tsx`                       |   412 | Done: Options stage wrapper lives outside the app shell while delegating the static center preview canvas. Keep it settings orchestration-only; ProjectState actions own mutation and persistence/export rules stay elsewhere.                                                                                                                                     |
...
| `utils/fabrication.ts`                                        |   692 | Split validation/export and package generation next only with stronger gates. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, readiness sampling/tolerance math lives in `utils/fabricationReadiness.ts`, render-plan validation/z-order lives in `utils/fabricationRenderPlan.ts`, Blueprint SVG rendering lives in `utils/fabricationBlueprintSvg.ts`, planetary/linkage sizing lives in `utils/fabricationSizing.ts`, import-free PDF document primitives live in `utils/simplePdf.ts`, character cut-sheet layout lives in `utils/fabricationCharacterPrintLayout.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
...
2. **Stage components**
   ...
   - Done: `components/stages/options/Options.tsx` owns the Options stage wrapper outside `App.tsx`, while `OptionsPreviewCanvas.tsx` owns the static center preview; Options still receives ProjectState/actions and does not own persistence or export rules.
