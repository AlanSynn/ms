...
| File                                                          | Lines | Decision                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------- | ----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `App.tsx`                                                     |   976 | First split continues. Command keyboard binding, boot/cache lifecycle, project history, autosave/workspace persistence, project/session command actions, and the stage router are extracted; keep shrinking by behavior-preserving seams only. No redesign mixed into extraction. |
...
2. **Stage components**
   ...
   - Next lowest-risk seam: extract remaining shell/workspace chrome or selection-derived action helpers after command/session, persistence, Foundry renderer, stage router, and boot/cache contracts stay green. Shared geometry/material/cache helpers already live in `utils/threeResourceKit.ts`; keep shared fabrication/physics contracts centralized.
