...
- Team consensus: completed narrow `AssemblyGuide` playback/model extractions, then approved cycle-free `utils/fabricationProfiles.ts`, `utils/numberFormat.ts`, and `utils/fabricationStackModel.ts` pure fabrication seams; defer `ThreePuppetPreview`, broad `utils/project.ts`, broad `utils/fabrication.ts`, and any further `TrackingModal` media-flow movement until stronger targeted harnesses exist.
...
| `utils/fabrication.ts`                                        |  1189 | Split manifest lookup, render plan, validation/export. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
