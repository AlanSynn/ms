...
| `components/stages/assembly/AssemblyGuide.tsx`                |   190 | Done: Assembly Guide stage wrapper owns live recipe derivation, assembly mode/lane state, export/print callbacks, and pane composition only. Keep it orchestration-only; pane JSX stays in Assembly leaf components, playback/reset timing stays in `useAssemblyGuidePlayback.ts`, and recipe/stack rules stay in utils/fabrication and utils/assemblyPlayback. |
