...
- `App.tsx` is now a 313-line composition shell after mechanism action and stage navigation extraction; the next App seams are stage-adjacent preview/control splits, not renderer or fabrication logic.
...
- Completed low-risk App stage navigation seam: stage handoff gate, recovery dispatch, and stage-open status now live in `utils/appStageNavigation.ts`.
- Next safe production seams, in order: split Assembly playback/view sections, then split pure `utils/fabrication.ts` concerns behind golden-master output hashes.
...
| File                                                          | Lines | Decision                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------- | ----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `App.tsx`                                                     |   313 | Composition shell. App wires state/action hooks into the shell without owning shell markup, renderer geometry, mechanism fitting, fabrication rules, export body logic, or stage handoff side effects. Command keyboard binding, boot/cache lifecycle, project history, autosave/workspace persistence, project/session command actions, derived selection state, path draw/tracking actions, character import/review actions, mechanism mutation/export/fitting actions, workspace player dock, shared playback loop, modal inert effect, starter image template assets, stage navigation, stage router, workspace status, and workspace shell chrome are extracted; keep shrinking by behavior-preserving seams only. No redesign mixed into extraction. |
...
| `utils/appStageNavigation.ts`                                  |    37 | Done: stage handoff gate, recovery dispatch, and stage-open status live outside App. Keep it side-effect wiring only; handoff rules stay in `utils/project.ts`, labels stay shell metadata, and no stage UI or ProjectState mutation rules belong here. |
...
| `components/stages/assembly/AssemblyGuide.tsx`                |   231 | Done: Assembly Guide stage wrapper owns live recipe derivation, assembly mode/lane state, shared playback timing, export/print callbacks, and pane composition only. Keep it orchestration-only; pane JSX stays in Assembly leaf components and recipe/stack rules stay in utils/fabrication and utils/assemblyPlayback. |
...
| `components/stages/assembly/AssemblyCanvasPane.tsx`           |    61 | Done: Assembly center canvas chooser owns only the mechanism/character workbench mount and empty state. Keep simulation drawing inside AssemblyWorkbench and character assembly workbench. |
...
1. **Command and app shell seams**
   ...
   - Done: `utils/appStageNavigation.ts` owns stage handoff gate, recovery dispatch, and stage-open status outside `App.tsx`; it stays side-effect wiring only and delegates rules to `utils/project.ts`.
