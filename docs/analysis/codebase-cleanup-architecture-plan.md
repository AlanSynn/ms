...
| `utils/fabrication.ts`                                        |   348 | Split validation/export and package generation next only with stronger gates. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, readiness sampling/tolerance math lives in `utils/fabricationReadiness.ts`, render-plan validation/z-order lives in `utils/fabricationRenderPlan.ts`, package recipe derivation lives in `utils/fabricationRecipes.ts`, character SVG/PDF/STL artifacts live in `utils/fabricationCustomParts.ts`, assembly guide HTML/PDF artifacts live in `utils/fabricationAssemblyGuide.ts`, Blueprint SVG rendering lives in `utils/fabricationBlueprintSvg.ts`, planetary/linkage sizing lives in `utils/fabricationSizing.ts`, import-free PDF document primitives live in `utils/simplePdf.ts`, character cut-sheet layout lives in `utils/fabricationCharacterPrintLayout.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
