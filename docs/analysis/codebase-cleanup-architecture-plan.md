...
- `App.tsx` is now a 324-line composition shell after mechanism action extraction; the next App seam is stage navigation, not renderer or fabrication logic.
...
- Next safe production seams, in order: extract App stage navigation, split Blueprint preview/inspector sections, split Assembly playback/view sections, then split pure `utils/fabrication.ts` concerns behind golden-master output hashes.
