...
2. **Stage components**
   ...
   - Next lowest-risk stage seam: continue with App boot/cache or persistence command seams after the Foundry renderer contracts stay green. Shared geometry/material/cache helpers already live in `utils/threeResourceKit.ts`; keep shared fabrication/physics contracts centralized.
