...
| `utils/fabricationCustomParts.ts`                             |   135 | Done: character custom-parts SVG, PDF, and STL artifact generation live outside the broad fabrication runtime. Keep it artifact-only: no package generation, validation, mechanism recipes, DOM, React, Three, or renderer ownership. |
| `utils/fabricationAssemblyGuide.ts`                           |   113 | Done: assembly guide exploded SVG, printable HTML, and PDF artifact generation live outside the broad fabrication runtime. Keep it artifact-only: no package generation, validation, cut-sheet/custom-parts output, DOM, React, Three, or renderer ownership. |
