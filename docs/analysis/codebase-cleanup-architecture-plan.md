...
| `utils/fabrication.ts`                                        |   939 | Split manifest lookup, validation/export, and package generation. Keep as the stable public facade for fabrication helpers; gear/ring profile geometry now lives in `utils/fabricationProfiles.ts`, stack/linkage label modeling lives in `utils/fabricationStackModel.ts`, readiness sampling/tolerance math lives in `utils/fabricationReadiness.ts`, render-plan validation/z-order lives in `utils/fabricationRenderPlan.ts`, Blueprint SVG rendering lives in `utils/fabricationBlueprintSvg.ts`, and fabrication rules still come from `fabrication/generate_fabrication_templates.py` plus `utils/fabricationContract.ts`. |
