...
test('character → path → foundry → design → blueprint runs end-to-end in browser', async ({ page }) => {
  ...
  await expect(page.getByTestId('foundry-mechanism-library')).toContainText('Use:');
  ...
  await expectProjectCounts(page, 10, 1, 1);
  ...
  await expect(page.getByTestId('prefab-assembly-steps')).toContainText('Set ground pivots');
  await expect(page.getByTestId('prefab-assembly-steps')).toContainText('L2');
  await expect(page.getByTestId('prefab-assembly-steps')).toContainText('L4');
...
test('Character tab processing controls route to real browser workflows', async ({ page }) => {
  ...
  await page.goto('/');
  await openCharacterScreen(page);
  await expect(page.getByTestId('character-processing-panel')).toContainText('Import tools');
  await expect(page.getByTestId('character-preview-pane')).toBeVisible();
  ...
  await page.getByRole('button', { name: 'Skip to editor' }).click();
  await page.getByText('Import tools').click();
...
test('Options parity updates workspace UI, canvas context, and blueprint defaults', async ({ page }) => {
  ...
  await page.getByLabel('Debug visuals').check();
  await page.getByRole('button', { name: /Path Editor/i }).click();
  ...
  await page.getByLabel('Processing details').check();
...
test('Recommendation sheet applies a distinct mechanism and blueprint recipe', async ({ page }) => {
  ...
  await expect(page.getByTestId('recommendation-sheet')).toContainText('Mechanism Recommendations for Right lower arm');
...
test('Workflow tabs keep left workflow, center canvas, and right inspector roles', async ({ page }) => {
  ...
  await clickStage(page, 'Blueprint');
  await assertPaneContract('Generate', 'Letter sheet', 'Cut sheet', '.blueprint-document-preview', '.blueprint-document-preview');
  await page.getByRole('button', { name: /Generate package/i }).click();
  await page.getByRole('button', { name: 'Assembly guide', exact: true }).click();
  await assertPaneContract('Build', 'Exploded view', 'Assembly', '.assembly-guide-web-preview', '.assembly-guide-web-preview');
...
test('Mechanism Design library chips, target filters, delete, and enabled export work', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'piston', exact: true }).click();
  await expectProjectCounts(page, 10, 1, 2);
