...
assert(telemetryTransportSource.includes("makeBufferedBatches(beacon, beacon || checkpoint)") && telemetryTransportSource.includes("!newSnapshotKeys.has(item.snapshotKey)"), "normal snapshot batches yield between durable writes while exit drains fully and constrained collapse preserves current-generation chunks");
