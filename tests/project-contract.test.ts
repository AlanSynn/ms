...
assert(appText.includes('useAppCharacterImportActions({') && !appText.includes('const importCharacterPackage =') && !appText.includes('const importProject =') && !appText.includes('const acceptPendingCharacter =') && appCharacterImportActionsHookText.includes('loadCharacterPackage') && appCharacterImportActionsHookText.includes('loadProjectSnapshot') && appCharacterImportActionsHookText.includes('setProject(pendingCharacter.project, { resetHistory: true })') && appCharacterImportActionsHookText.includes('returnStage: "character"') && !appCharacterImportActionsHookText.includes('processImageWithWebOnnx'), 'App delegates package/project import review while the image-recognition path stays removed');
...
assert(appCharacterImportActionsHookText.includes('returnStage: "character"') && appCharacterImportActionsHookText.includes('loadCharacterPackage(files)') && !appCharacterImportActionsHookText.includes('AI could not load'), 'accepted local character packages stay in Character without an AI failure path');
