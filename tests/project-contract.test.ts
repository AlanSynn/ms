...
assert(normalizedCodebaseCleanupPlan.includes('`App.tsx` | 2311') && normalizedCodebaseCleanupPlan.includes('Options and AssemblyGuide stage wrapper seams are extracted'), 'cleanup plan records the current App.tsx hotspot and completed Options/Assembly stage seams');
...
assert(normalizedCodebaseCleanupPlan.includes('`components/stages/foundry/ThreeFoundryPreview.tsx` | 2082') && normalizedCodebaseCleanupPlan.includes('shared Foundry/Design Three renderer seam lives outside the app shell'), 'cleanup plan records the extracted shared Foundry Three renderer seam');
...
assert(normalizedCodebaseCleanupPlan.includes('`components/stages/foundry/MechanismFoundry.tsx` | 1571') && normalizedCodebaseCleanupPlan.includes('Mechanism Foundry stage wrapper lives outside the app shell'), 'cleanup plan records the extracted MechanismFoundry stage seam');
...
assert(mechanismFoundryText.includes('foundry-parametric-editor') && appText.includes('design-parametric-editor'), 'Foundry and Design both mount the same compact parametric mechanism editor');
...
assert(mechanismFoundryText.includes('<MechanismParametricEditor') && appText.includes('<MechanismParametricEditor') && mechanismParametricEditorText.includes('gearTrainPitchRadii') && mechanismParametricEditorText.includes('defaultCamProfileSamples'), 'Foundry and Design delegate compact parametric gear/link/cam controls to a mechanism stage seam');
assert(mechanismFoundryText.includes('MECHANISM_PARAM_META') && appText.includes('MECHANISM_PARAM_META') && mechanismParamPolicyText.includes('shouldShowMechanismParam') && mechanismParamPolicyText.includes('clampMechanismParam'), 'Foundry and Design delegate legacy numeric mechanism parameter policy to a pure mechanism stage helper');
...
assert(((appText + mechanismFoundryText).match(/<ThreeFoundryPreview/g) ?? []).length >= 2, 'Foundry and Mechanism Design both mount ThreeFoundryPreview');
assert(appText.includes('data-renderer-source="ThreeFoundryPreview"') && appText.includes('data-shared-with="foundry-preview"') && mechanismFoundryText.includes('<ThreeFoundryPreview'), 'Mechanism Design advertises that its mechanism view is shared with Foundry while Foundry mounts the same preview component');
...
assert(foundry3dText.includes('data-three-stack-source') && appText.includes('Design explode stack'), 'Mechanism Design exposes Foundry fabrication stack provenance and explode controls for browser verification');
...
assert(!mechanismRecommendationsText.includes('fitMechanismGeneratedPathToPath') && !appText.includes('fitMechanismGeneratedPathToPath'), 'mechanism recommendations must not center-shift physical templates away from hole-snapped anchors');
...
assert(foundry3dText.includes("scene.remove(old)") && foundry3dText.includes("disposeThreeObject(old)"), 'Foundry disposes noncached dynamic resources when replacing animation groups');
...
assert(appUiText.includes('data-testid="onnx-cache-status"') && appText.includes('warmWebOnnxCache'), 'status bar exposes ONNX cache/download status');
...
assert(appText.includes('warmWebOnnxCache(publishBootStatus)') && appText.includes('finishBootLoader()') && appText.includes('document.getElementById("boot-loader")?.remove()'), 'React keeps the static boot loader through AI model warmup before opening the editor');
...
assert(appUiText.includes('getting-started-card-guided') && appUiText.includes('Open Guide') && appUiText.includes('guided-project-library') && appUiText.includes('guided-project-card-${lesson.id}'), 'Getting Started exposes guided projects through an explicit Guide tile while keeping open exploration visible');
...
assert(!appText.includes('<WelcomeDialog') && !appUiText.includes('WelcomeDialog') && !appText.includes('setShowWelcome') && !appText.includes('setShowGettingStarted(!hideNextTime)') && appText.includes('const [showGettingStarted, setShowGettingStarted] = useState(false)'), 'Startup uses the static boot loader only; the editor opens directly to Character and Guide is explicit');
assert(characterImportControlsText.includes('onOpenGettingStarted') && characterSelectionText.includes('<CharacterImportControls'), 'Character tab can reopen Getting Started through extracted import controls without owning its starter gallery');
...
assert(characterImportControlsText.includes('character-import-controls') && characterImportControlsText.includes('blank-package-input') && characterImportControlsText.includes('onnx-input') && characterImportControlsText.includes('onboarding-import-input') && characterImportControlsText.includes('Keep compatible mechanisms') && characterSelectionText.includes('<CharacterImportControls'), 'Character import file controls live in the extracted import-controls seam with stable chooser ids');
...
assert(mechanismFoundryText.includes('data-testid="foundry-visible-sensemaking"') && appText.includes('data-testid="design-visible-sensemaking"'), 'Foundry and Design show compact visible sensemaking by default instead of hiding all meaning behind details');
assert(`${appText}
${mechanismFoundryText}`.includes('data-sensemaking-evidence') && `${appText}
${mechanismFoundryText}`.includes('data-sensemaking-answer') && `${appText}
...
assert(blueprintCanvasStart >= 0 && blueprintInspectorStart > blueprintCanvasStart, 'Blueprint layout exposes web-first board preview and inspector slots');
...
assert(blueprintExportText.includes('data-testid="blueprint-sensemaking-label"') && assemblyWorkbenchText.includes('data-testid="assembly-workbench-sensemaking"') && assemblyGuideText.includes('data-testid="assembly-sensemaking-label"'), 'Blueprint and Assembly reuse mechanism sensemaking metadata for compact visual cues');
...
assert(assemblyBlock.includes('stepProgressRef') && assemblyBlock.includes('window.requestAnimationFrame(tick)'), 'Assembly playback advances with rAF progress instead of only jumping static steps');
assert(assemblyBlock.includes('setStepCount(activeStepCount)') && assemblyBlock.includes('setStepIndex') && !assemblyBlock.includes('data-testid="assembly-player-overlay"'), 'Assembly tab reports step count to the shared player and no longer renders a duplicate local player');
