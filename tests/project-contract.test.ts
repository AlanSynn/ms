...
const viewSessionBeforeJson = JSON.stringify(projectionProject);
const studioCamera = lockCameraToStudio(projection);
const toyCamera = unlockCameraPreview(projection);
const physicsCamera = cameraForLens(projection, 'physics');
const resetToyCamera = resetCameraForLens(projection, 'toy-stage');
assert(cameraSessionIsViewOnly(studioCamera) && studioCamera.locked && !studioCamera.orbitEnabled, 'studio lens camera is locked and view-only');
assert(cameraSessionIsViewOnly(toyCamera) && !toyCamera.locked && toyCamera.orbitEnabled, 'toy-stage camera unlock stays a view-only preview');
assert.equal(physicsCamera.presetId, 'depth-inspect', 'physics lens uses the fixed depth inspection camera');
assert.equal(resetToyCamera.presetId, 'toy-stage', 'camera reset restores the selected lens preset');
assert.equal(JSON.stringify(projectionProject), viewSessionBeforeJson, 'lens and camera session helpers do not mutate ProjectState');
...
assert.equal(JSON.stringify(projectionProject), physicsBeforeJson, 'physics sidecar does not mutate ProjectState');
assert.deepEqual(buildKinematicPhysicsSession(projectionProject, projection, Math.PI / 3), physicsSession, 'physics sidecar is deterministic for the same projection and angle');
assert.equal(physicsSession.version, 1, 'physics sidecar exposes schema version');
assert.equal(physicsSession.sourceProjectionVersion, projection.version, 'physics sidecar records projection schema source');
assert(physicsSession.summary.bodyCount >= projection.nodes.filter(node => ['part', 'joint', 'mechanism', 'hardware'].includes(node.sourceType)).length, 'physics sidecar creates bodies from projected scene nodes');
assert(physicsSession.summary.constraintCount >= projectionProject.mechanisms.filter(mechanism => mechanism.visible && mechanism.enabled !== false).length, 'physics sidecar samples mechanism constraints');
assert(physicsSession.bodies.some(body => body.sourceType === 'mechanism-state' && body.label.includes('effector')), 'physics sidecar includes kinematic end-effector samples');
assert(physicsSession.summary.maxSpeed > 0, 'physics sidecar derives non-zero kinematic velocity from the current mechanism animation');
