...
assert(webOnnxWorkerText.includes('MAX_SOURCE_TEXTURE_EDGE = 512') && webOnnxWorkerText.includes('MAX_SOURCE_TEXTURE_BYTES = 256 * 1_024'), 'import review and ProjectState reuse a Chromebook-bounded display texture');
assert(!webOnnxWorkerText.includes('MAX_PART_TEXTURE_BYTES') && !webOnnxWorkerText.includes('textureUrl: crop.textureUrl') && !webOnnxWorkerText.includes('maskUrl: crop.maskUrl'), 'ONNX parts keep editable contours and source frames without duplicating embedded art and mask bytes');
...
assert(characterImportOverlaysText.includes('character-import-review-preview') && characterSelectionText.includes('<CharacterImportStatusDock') && characterSelectionText.includes('reviewedProject={reviewedProject}') && characterSelectionText.includes('project={project}') && characterSelectionText.includes('<CharacterImportReviewDialog') && characterImportOverlaysText.includes('showImportChecks = project.settings.debugVisuals'), 'Character imports preview optimized source art in the approval overlay without building pending WebGL geometry, while checks stay dev-only');
...
assert(assemblyThreePreviewText.includes('const assemblyStepExplode') && assemblyThreePreviewText.includes('explode={assemblyStepExplode(sceneFrame.motion, progress)}') && assemblyThreePreviewText.includes('Build animation') && !assemblyThreePreviewText.includes('return 0.72') && !assemblyCanvasPaneText.includes('playing={playing}'), 'Assembly has its own step-local explode amount while Foundry keeps separate slider control');
assert(foundryThreeRenderLayersText.includes('foundryAssemblyVisibleLayerIndexes') && foundryThreeRenderLayersText.includes('layer.stackIndex === frame.stepIndex - 1') && foundryThreeRenderLayersText.includes('pin.layerIndexes.forEach'), 'Assembly renders the current recipe stack and its required fastener closure instead of the completed mechanism at every step');
assert(assemblySceneFrameText.includes('activeScenePoints') && assemblySceneFrameText.includes('floatingReferencePoints') && foundryAssemblySceneOverlayText.includes('frame.kind === "character"') && foundryAssemblySceneOverlayText.includes('(frame.activeScenePoints ?? []).map(scenePointToPreviewPoint)') && threeFoundryPreviewText.includes('assemblyLift'), 'Character assembly feeds art pins into the Foundry renderer through the scene-to-Foundry coordinate seam with its own z-only build separation');
...
assert(assemblyPlaybackText.includes("motion: 'explode_z' as const") && assemblyPlaybackText.includes("motion: 'mount_travel_xy'") && assemblyPlaybackText.includes("motion: 'connect_travel_xy'") && assemblyPlaybackText.includes("motion: 'scrub_time'"), 'Assembly playback declares its z-only stack separation separately from Foundry slider state');
...
assert(assemblySceneFrameText.includes('motion: step.motion') && assemblySceneFrameText.includes("step.motion === 'explode_z'") && assemblySceneFrameText.includes("step.phase === 'test-character'"), 'Assembly scene frame visualizes parts, mounting, character connection, and test motion through independent canonical frame motion states');
