...
assert.equal(packageJson.version, '0.0.18', 'classroom motion copy release advances without moving existing release tags');
...
assert(appControllerSource.includes('useProjectHistory(createEmptyProject)') && appProjectHistoryHookText.includes('return { present: withSessionPerformance(createInitialProject()), past: [], future: [] }'), 'App history initializes empty; candidate discovery never chooses the student project');
