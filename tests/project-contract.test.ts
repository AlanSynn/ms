...
assert(bundleBudgetSource.includes('excludesOptionalAiModel: true'), 'bundle evidence states that the opt-in AI model is excluded');
...
assert(performanceWorkflowText.includes('bun run test:chromebook-audit') && performanceWorkflowText.includes('bun run test:chromebook-audit:full') && performanceWorkflowText.includes('bun run test:chromebook-audit:real-ai'), 'performance workflow keeps short, full, and real-AI scopes explicit');
...
assert(chromebookAuditDoc.includes('396,784 bytes') && chromebookAuditDoc.includes('899,787 bytes'), 'audit document records the checked production bundle evidence');
...
assert(appText.includes('STARTER_IMAGE_TEMPLATES') && !appText.includes('girl.png?url') && starterImageTemplatesText.includes('girl.png?url') && starterImageTemplatesText.includes('boy.png?url') && !starterImageTemplatesText.includes('boy.PNG?url') && !starterImageTemplatesText.includes('girl-thumb.png?url') && !starterImageTemplatesText.includes('boy-thumb.png?url'), 'App delegates starter image template assets to the updated Downloads-derived PNG resources');
...
assert(designWorkflowPanelText.includes('const DesignRecommendationControl') && designWorkflowPanelText.includes('useState(false)') && designWorkflowPanelText.includes('<MechanismRecommendationSheet') && designWorkflowPanelText.includes('onApply(mechanism)') && designWorkflowPanelText.includes('prepareMechanismRecommendationWorker') && !mechanismDesignText.includes('useState(false)') && !appWorkspaceShellText.includes('<MechanismRecommendationSheet') && mechanismRecommendationSheetText.includes('createMechanismRecommendationWorkerClient') && mechanismRecommendationSheetText.includes('mechanismWithGeneratedPath') && mechanismRecommendationJobText.includes('buildMechanismRecommendations') && mechanismRecommendationWorkerClientText.includes('generationId') && mechanismRecommendationWorkerClientText.includes('type: "warm"') && mechanismRecommendationWorkerText.includes('runMechanismRecommendationJob'), 'the Design recommendation control owns modal visibility and prewarms only the tiny worker entry before the generation-checked heavy job');
assert(appMechanismActionsHookText.includes('createMechanismOptimizerWorkerClient') && mechanismOptimizerJobText.includes('runMechanismOptimizerSearch') && mechanismOptimizerJobText.includes('fitMechanismToTargetPath') && mechanismOptimizerWorkerClientText.includes('generationId') && mechanismOptimizerWorkerClientText.includes("type: 'warm'") && mechanismOptimizerWorkerText.includes('runMechanismOptimizerJob(data.input') && designInspectorPanelText.includes('prepareMechanismOptimizerWorker') && !appMechanismActionsHookText.includes('for (let i = 0; i < iterations; i++)'), 'Design fitting prewarms only the tiny worker entry, then runs deterministic search and final path fitting in a cancellable generation-checked worker');
...
assert(appText.includes('useAppOnnxBootstrap') && appOnnxBootstrapText.includes('warmWebOnnxCacheInWorker(setOnnxCacheStatus)') && appOnnxBootstrapText.includes('stage: "missing"') && !appOnnxBootstrapText.includes('requestIdleCallback') && !appOnnxBootstrapText.includes('setTimeout(') && !appOnnxBootstrapText.includes('finishBootLoader()') && indexTsxText.includes("void import('./App')") && indexTsxText.includes('releaseBootLoader'), 'the workspace releases the static boot loader without starting an ONNX request until explicit AI preparation');
...
assert(gettingStartedDialogCommandSource.includes('getting-started-card-humanoid') && gettingStartedDialogCommandSource.includes('getting-started-card-image') && !gettingStartedDialogCommandSource.includes('getting-started-card-package') && gettingStartedDialogCommandSource.includes('showStarterImages && starterTemplates.map') && gettingStartedDialogCommandSource.includes("data-starter-images={showStarterImages ? 'visible' : 'hidden'}") && appWorkspaceShellText.includes('showStarterImages: boolean') && appWorkspaceShellText.includes('showStarterImages={showStarterImages}') && appControllerSource.includes('showStarterImages: false') && starterImageTemplatesText.includes('id: "girl"') && starterImageTemplatesText.includes('id: "boy"'), 'Getting Started keeps Boy/Girl starter resources available but hides their tiles on the first landing');
...
assert(indexText.includes('.starter-thumb { width: 2.25rem; height: 2.25rem;') && indexText.includes('.getting-started-dialog .template-gallery .starter-thumb { width: 6rem; height: 6rem;') && indexText.includes('transform: scale(1.22)'), 'reserved starter thumbnail styling remains available if the secondary starter surface is re-enabled');
