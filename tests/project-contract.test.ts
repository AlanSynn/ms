...
assert(gettingStartedDialogCommandSource.includes('onSample(starterRigPreview)') && gettingStartedDialogCommandSource.includes('onLesson(lesson.id, previewProjects[lesson.id]') && appProjectCommandsHookText.includes('preparedProject ?? createSampleProject()') && appProjectCommandsHookText.includes('preparedProject ?? createLessonProject(lesson.id)'), 'starter clicks reuse their already-rendered serializable ProjectState instead of rebuilding the project on the interaction path');
