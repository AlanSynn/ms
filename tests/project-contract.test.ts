...
import { animatedPartsForProject, describeMotionChain, mechanismBindingWarnings, mechanismPathFitIsUsable, motionAnchorJointIds, motionChainRootJointIds, motionPathReadiness, motionPreviewForPath, motionPreviewForProject, motionPreviewForTarget, pointOnGeneratedMechanismPath, pointOnProjectPath, preferredMotionJointId } from '../utils/motion';
...
import { ownerLocalToScene } from '../utils/artwork';
import { preparePuppetPartTopology } from '../runtime/render/puppetPartTopology';
...
import { projectStateFromPortableDocument, serializeProjectCompact } from '../utils/projectSerialization';

const deserializePortableProject = (project: ProjectState) =>
  projectStateFromPortableDocument(JSON.parse(serializeProject(project))) as ProjectState;
...
assert(
  appControllerSource.includes('useAppCommandBindings({ commandHandlers, disabled: modalOpen || !!support.surface || Boolean(versions.view.preview) })') &&
...
assert(APP_COMMANDS.filter(command => command.menuVisible !== false).every(command => APP_MENU_GROUPS.some(group => group.commandIds.some(id => id === command.id))), 'every classroom-visible command is rendered by a menu group');
assert(APP_COMMANDS.filter(command => command.menuVisible === false).every(command => !APP_MENU_GROUPS.some(group => group.commandIds.some(id => id === command.id)) && !command.shortcuts?.length), 'legacy duplicate project commands retain their ids without menu or shortcut pathways');
assert.deepEqual(['project.new', 'project.save', 'project.open', 'project.recoverAutosave'].map(id => commandById(id as typeof expectedAppCommandIds[number]).label), ['New Project', 'Save Project', 'Open Project', 'Recover browser backup'], 'Project exposes the canonical classroom project commands');
...
assert(appWorkspaceShellCommandSource.includes('suspendStageContent={booting || showGettingStarted || support.startupAnnouncement}') && !classroomStageModulesSource.includes('lazy(') && classroomStageModulesSource.includes('if (!pathModule) throw loadPath()') && classroomStageModulesSource.includes('return <Component {...props} />') && !classroomStageModulesSource.includes('loadCharacterStage') && classroomStageModulesSource.includes('preloadNextClassroomStage') && classroomStageModulesSource.includes('case "character"') && classroomStageModulesSource.includes('foundryPromise = undefined') && classroomStageModulesSource.includes('pathPromise = undefined') && appStageRouterCommandSource.includes('import { CharacterSelection }'), 'Getting Started keeps hidden WebGL unmounted while the small Character shell is immediately available and retryable Suspense adapters warm only the next classroom stage');
...
assert(playwrightConfigText.includes('reuseExistingServer: false'), 'browser tests never reuse an unrelated process occupying the preview port');
...
assert.equal(packageJson.version, '0.0.16', 'painting release version advances without moving existing release tags');
...
assert.equal(packageJson.scripts.build, 'tsc && bun scripts/check-no-image-recognition.mjs --source && vite build && bun scripts/check-no-image-recognition.mjs --dist && bun scripts/check-feedback-boundary.mjs', 'browser build keeps Vite for Rapier and hard-fails if image recognition returns');
...
assert(agentsContract.includes('Classroom return is file-first') && agentsContract.includes('Browser backups are explicit secondary recovery candidates'), 'AGENTS.md locks portable file return and explicit browser recovery');
...
const customAssessment = loadProjectSnapshot({
  ...deserializePortableProject(emptyProject),
...
assert.equal(occupiedFourBarRecommendation, undefined, 'recommendations do not offer a second four-bar when the selected motion path already has an explicit mechanism output');
...
const snapshotBeforeProject = serializeProjectCompact(sample);
...
assert.equal(serializeProjectCompact(sample), snapshotBeforeProject, 'mechanism snapshot builder does not mutate ProjectState');
...
const goldenMaster = {
  project: deserializePortableProject(stableProjectForGoldenMaster(goldenSample)),
  lesson: deserializePortableProject(stableProjectForGoldenMaster(goldenLesson)),
...
assert.deepEqual(
  ...
  {
    // Intentional contract change: portable v2 normalization adds revision/path-order defaults and
...
assert(
  ...
  && fabricationRuntimeText.includes("from './fabricationBlueprintPdf'")
  ...
  'fabrication runtime consumes extracted profile/stack/render/recipe/custom-parts/assembly-guide/true-scale Blueprint helpers, PDF primitives, and Blueprint SVG renderers from focused seams'
);
[
  './fabrication',
  './sanitize',
...
assert.deepEqual(
  ...
  ['../types', './buildPlan', './fabricationContract', './fabricationRecipes', './fabricationStackModel', './kinematics', './simplePdf'].sort(),
...
assert.deepEqual(
  ...
  ['../types', './coordinates', './fabricationContract', './fabricationReadiness', './fabricationRenderPlan', './fabricationStackModel', './kinematics', './mechanismBindings', './mechanismReference', './motion'].sort(),
...
assert.deepEqual(
  ...
  ['../types', './buildPlan', './fabricationBlueprintPdf'].sort(),
  'fabricationCutSheetPdf is a narrow compatibility facade over the true-scale Blueprint PDF seam'
);
[
  './fabrication',
  './project',
...
assert.deepEqual(
  ...
  ['../types', './buildPlan', './characterFabricationHoles', './coordinates', './fabricationCharacterPrintLayout', './fabricationOutlineIssues', './numberFormat', './partGeometry', './projectSerialization', './simplePdf'].sort(),
...
assert.deepEqual(
  ...
  ['../types', './characterFabricationHoles', './coordinates', './partGeometry'].sort(),
...
const assemblyLocalPartsPreviewText = readFileSync(join(process.cwd(), 'components', 'stages', 'assembly', 'AssemblyLocalPartsPreview.tsx'), 'utf8');
...
const cutOutlineEditorText = readFileSync(join(process.cwd(), 'components', 'stages', 'character', 'ShapeEditor.tsx'), 'utf8');
...
const replacementWindow = Object.getOwnPropertyDescriptor(globalThis, 'window');
const replacementQuestions: string[] = [];
try {
  Object.defineProperty(globalThis, 'window', { configurable: true, value: { confirm: (message: string) => { replacementQuestions.push(message); return false; } } });
  mechanismUpdateHarness.actions.updateMechanism('mech-1', { targetPartId: 'head' });
  assert.equal(mechanismUpdateHarness.dispatches.length, 0, 'canceling binding replacement preserves the previous fit');
  Object.defineProperty(globalThis, 'window', { configurable: true, value: { confirm: (message: string) => { replacementQuestions.push(message); return true; } } });
  mechanismUpdateHarness.actions.updateMechanism('mech-1', { targetPartId: 'head' });
  assert.equal(replacementQuestions.length, 2, 'each binding replacement needs an explicit decision');
} finally {
  if (replacementWindow) Object.defineProperty(globalThis, 'window', replacementWindow);
...
const recommendationOwnerProject = createSampleProject({ includeMechanism: true });
const recommendationOwner = recommendationOwnerProject.mechanisms[0];
const recommendationPathId = recommendationOwner.targetPathId;
assert(recommendationPathId, 'recommendation replacement fixture has an owned path');
const recommendationCandidate = {
  ...createDefaultMechanism('gear_linkage', 'recommendation-replacement-contract'),
  targetPathId: recommendationPathId,
  targetPartId: recommendationOwner.targetPartId,
  targetSceneObjectId: recommendationOwner.targetSceneObjectId,
  targetAnchorJointId: recommendationOwner.targetAnchorJointId,
  source: 'optimized' as const,
  presetId: 'recommendation-gear_linkage',
};
const recommendationReplacementHarness = renderMechanismActionHarness({
  project: recommendationOwnerProject,
  selectedMechanism: recommendationOwner,
  selectedPath: recommendationOwnerProject.paths[recommendationPathId],
});
recommendationReplacementHarness.actions.applyRecommendedMechanism(recommendationCandidate);
const recommendationReplacementDispatch = recommendationReplacementHarness.dispatches[0] as {
  type: 'upsert_mechanism';
  mechanism: MechanismConfig;
};
assert.equal(recommendationReplacementDispatch.type, 'upsert_mechanism', 'recommendation replacement uses the canonical mechanism action');
assert.equal(recommendationReplacementDispatch.mechanism.id, recommendationOwner.id, 'recommendation replacement preserves the existing owner id');
const recommendationReplacedProject = applyProjectAction(recommendationOwnerProject, recommendationReplacementDispatch);
assert.equal(recommendationReplacedProject.mechanisms.length, recommendationOwnerProject.mechanisms.length, 'recommendation replacement does not append a duplicate mechanism');
assert.equal(recommendationReplacedProject.mechanisms.find((mechanism) => mechanism.id === recommendationOwner.id)?.type, 'gear_linkage', 'recommendation replacement updates the existing owner in place');
...
assert.equal(designPathFitMode, '', 'Design geometry edits do not start asynchronous path fitting');
assert.equal(designPathFitDispatch.mechanism.anchorX, 9_999, 'Design commits the entered path-bound geometry value immediately');
assert.notEqual(
  designPathFitDispatch.mechanism.fabricationMetadata?.pathFit?.status,
  'fit',
  'Design invalidates a stale path fit after geometry edits',
);
assert.ok(
  mechanismBoardPlacementErrors(designPathFitProject, designPathFitDispatch.mechanism).length > 0,
  'Design preserves an invalid geometry edit so the existing validation surface can report it',
);
...
const designSheetFitDispatch = designSheetFitHarness.dispatches[0] as { mechanism: MechanismConfig };
assert.equal(designSheetFitMode, '', 'Unbound Design geometry edits do not start asynchronous sheet fitting');
assert.equal(designSheetFitDispatch.mechanism.anchorX, 9_999, 'Design commits the entered unbound geometry value immediately');
assert.ok(
  mechanismBoardPlacementErrors(designSheetFitProject, designSheetFitDispatch.mechanism).length > 0,
  'Unbound Design geometry edits remain visible to board validation',
);
...
assert.equal(designFailedFitHarness.dispatches.length, 1, 'manual Design geometry commits without waiting for a fit worker');
assert.equal(
  (designFailedFitHarness.dispatches[0] as { mechanism: MechanismConfig }).mechanism.anchorX,
  9_999,
  'a fit-worker failure cannot discard a manual Design geometry edit',
);
assert.equal(designFitFailure, '', 'manual Design geometry does not enter fit-worker rollback state');
...
assert.equal(supersededDesignFitFailure, '', 'sequential synchronous Design edits do not create a superseded fit failure');
assert.equal(designSupersededFitHarness.dispatches.length, 2, 'sequential Design edits each commit to canonical state');
assert(
...
assert(!foundryWorkflowPanelText.includes('from "lucide-react"') && foundryWorkflowPanelText.includes('{fitBusy ? "Cancel" : "Fit motion"}') && foundryWorkflowPanelText.includes('Use this mechanism'), 'Foundry cold stage keeps its primary actions result-first without mounting decorative SVG icon trees');
assert(
...
const conflictingExactFitProject: ProjectState = {
  ...starterSample,
  paths: { ...starterSample.paths, [exactFitPath.id]: exactFitPath },
};
const conflictingPathsBeforeFit = JSON.stringify(conflictingExactFitProject.paths);
assert.equal(motionPathReadiness(conflictingExactFitProject, exactFitPath).status, 'Conflict', 'two enabled paths on the same moving chain share one conflict status');
assert.equal(fitMechanismToTargetPath(conflictingExactFitProject, exactFourBarSeed, exactFitPath.id).fabricationMetadata?.pathFit?.status, 'rejected', 'fitting respects the same overlapping-path blocker as inventory and playback');
assert.equal(JSON.stringify(conflictingExactFitProject.paths), conflictingPathsBeforeFit, 'a blocked fit preserves both authored paths and their geometry');
const exactFitProject: ProjectState = {
  ...conflictingExactFitProject,
  paths: Object.fromEntries(Object.entries(conflictingExactFitProject.paths).map(([id, path]) =>
    [id, id === exactFitPath.id ? path : { ...path, enabled: false }],
  )),
};
...
const inactiveExactFitPaths = Object.fromEntries(Object.entries(exactFitProject.paths).map(([id, path]) =>
...
const reversedFit = fitMechanismToTargetPath(
  { ...exactFitProject, paths: { ...inactiveExactFitPaths, [reversedFitPath.id]: reversedFitPath } },
...
const shiftedFit = fitMechanismToTargetPath(
  { ...exactFitProject, paths: { ...inactiveExactFitPaths, [shiftedFitPath.id]: shiftedFitPath } },
...
const evenFit = fitMechanismToTargetPath(
  { ...exactFitProject, settings: { ...exactFitProject.settings, physicalKit: evenKit }, paths: { ...inactiveExactFitPaths, [evenPath.id]: evenPath } },
  evenFitSeed,
  evenPath.id,
);
...
assert(exporterText.includes("import { gearPathD } from './fabricationProfiles'"), 'blueprint/export gear rendering uses shared fabrication gear geometry');
...
assert(mechanismDesignText.includes('<DesignFoundryPreview') && designFoundryPreviewText.includes('export const DesignFoundryPreview') && designFoundryPreviewText.includes('\"design-shared-foundry-preview\"') && designFoundryPreviewText.includes('\"project-working-preview\"'), 'Mechanism Design owns a thin automata preview adapter instead of a separate legacy renderer');
...
assert(exporterText.includes("export { gearPathD } from './fabricationProfiles'") && !readFileSync('components/stages/foundry/MechanismLinkagePreview.tsx', 'utf8').includes('utils/exporter'), 'legacy gear-path export stays compatible while the viewport avoids loading serializers');
...
assert(
  motionText.includes('motionPathRuntimeCache') &&
    ...
    motionText.includes('prepared.affectedPartIds') &&
    motionText.includes('applyMotionTargetPose') &&
...
assert(appText.includes('useAppPathActions({') && !appText.includes('const setPathPoints =') && !appText.includes('setShowTracking(false);\n    setStage("path")') && appPathActionsHookText.includes('motionPathWithPoints(project, targetKind, targetId, points, source, timedPoints, pathId)') && readFileSync(join(process.cwd(), 'utils', 'pathEditing.ts'), 'utf8').includes('return validatePath({') && appPathActionsHookText.includes('ProjectMotionPath["source"] = "drawn"') && appPathActionsHookText.includes('setPathPoints(path, "tracked")') && appPathActionsHookText.includes('setStage("path")'), 'App delegates Path draw/tracking actions to useAppPathActions while preserving validated drawn/tracked path upserts');
assert(readFileSync(join(process.cwd(), 'utils', 'pathEditing.ts'), 'utf8').includes('closed: current?.closed ?? true'), 'new drawn/tracked paths default to closed loops while preserving existing open paths');
...
assert(workspacePlayerDockHookText.includes('const showsWorkspacePlayer =') && workspacePlayerDockHookText.includes('editorStage === "project"') && workspacePlayerDockHookText.includes('editorStage === "path"') && workspacePlayerDockHookText.includes('editorStage === "design"') && workspacePlayerDockHookText.includes('editorStage === "assembly"'), 'shared playback dock is restricted to working Project motion, Path, Mechanism Design, and Assembly');
...
assert(pathCanvasPaneText.includes('playback={{') && !pathCanvasPaneText.includes('playback={isPlaying ?') && pathCanvasPaneText.includes('sample: playbackSample') && pathEditorText.includes('playbackClock.getTimelineMs() ||') && pathEditorText.includes('motionTimelineMsForPhase(phase, playbackDurationMs)') && workspacePlaybackLoopHookText.includes('!isPlaying ||') && workspacePlaybackLoopHookText.includes('return () => playbackClock.stop()') && externalPlaybackClockText.includes('scheduler.cancelFrame(frameHandle)') && pathCanvasPaneText.includes('mechanisms={[]}'), 'Path keeps held/scrubbed elapsed-time sampling available after Pause while the shared playback loop gates idle animation and excludes mechanism geometry');
assert(pathCanvasPaneText.includes('drawMode={pathViewMode === "2d" && drawMode') && pathCanvasPaneText.includes('onDrawPoint={onDrawPoint}') && pathCanvasPaneText.includes('onSelectPathPoint={pathLocked ? undefined : onPathPointPick}') && pathCanvasPaneText.includes('onSelectPart={pathLocked ? undefined : (partId)') && pathCanvasPaneText.includes('onSelectSceneObject={pathLocked ? undefined : (objectId)') && pathCanvasPaneText.includes('onSelectJoint={pathLocked ? undefined : onJointPick}'), 'Path Editor keeps drawing and editable path handles on the shared front-view scene while a locked target cannot be reselected through the canvas');
...
assert(pathEditorText.includes('pointDragDraftRef.current = points') && pathEditorText.includes('const valid = gestureMatches()') && pathEditorText.includes('if (!valid || !dirty || !points || !identity?.path)') && pathEditorText.includes('setPathPoints(points, identity.path.source, undefined, identity.path.id)') && !pathEditorText.includes('freeDraftRef.current = next;\n    setPathPoints('), 'Path pointer moves stay transient; pointerup commits only a valid gesture to its captured path identity');
...
assert(pathCanvasPaneText.includes('const pathsToRender = React.useMemo(') && pathCanvasPaneText.includes('motionPaths.filter((path) => path.visible)') && pathCanvasPaneText.includes('paths={pathsToRender}') && pathCanvasPaneText.includes('selectedPathId={selectedPath?.id}'), 'Path Editor retains all visible authored paths in the shared preview with an explicit selected path identity');
...
assert(designFoundryPreviewText.includes('showPathPreview={showMechanismPath}') && designFoundryPreviewText.includes('pathTraces={sceneModel.foundryPreview.pointTraces}') && designFoundryPreviewText.includes('paths: showUserPath ? authoredPaths : []') && designFoundryPreviewText.includes('selectedPathId'), 'Mechanism Design routes fitted mechanism traces and all authored paths through the native Foundry preview seam');
...
assert(agentsContract.includes('Path Editor must render only character, skeleton, editable path') && agentsContract.includes('Project overview and Mechanism Design overlay character + authored paths + mechanism'), 'AGENTS.md locks tab-scoped rendering ownership for Path vs Mechanism Design');
...
assert(appText.includes('useProjectAutosave(project') && appControllerSource.includes('useColdAutosaveRecovery') && appControllerSource.includes('projectDecision: autosaveRecovery.decision') && coldAutosaveRecoveryHookText.includes('{ readOnly: true }') && !coldAutosaveRecoveryHookText.includes('setProject(') && coldAutosaveRecoveryHookText.includes('client.dispose()') && autosaveRecoveryWorkerClientText.includes('active.firstFrame') && autosaveRecoveryWorkerClientText.includes('active.secondFrame') && autosaveRecoveryWorkerClientText.includes('backend.readRecoverySnapshot()') && autosaveRecoveryWorkerClientText.includes('captureAutosaveRecoveryStorage') && autosaveRecoveryWorkerClientText.includes('autosaveRecoveryStorageIsCurrent') && autosaveRecoveryWorkerText.includes('await import(') && appProjectCommandsHookText.includes('createAutosaveRecoveryWorkerClient') && appProjectCommandsHookText.includes('readWorkspaceLayoutSnapshot') && appProjectCommandsHookText.includes('writeWorkspaceLayoutSnapshot') && appAutosaveHookText.includes('createAutosaveTransaction') && appAutosaveHookText.includes('pagehide') && appAutosaveHookText.includes('beforeunload') && projectPersistenceText.includes('readStorageWithLegacy') && projectPersistenceText.includes('migrateStorageValue') && projectAutosaveFormatText.includes('motionsmith.autosave'), 'cold boot preserves an empty ProjectState while a lazy recovery worker only discovers a candidate; explicit file/new/recover decisions authorize replacement and writes');
assert(appAutosaveHookText.includes('projectDecision') && appAutosaveHookText.includes('isAuthorized()') && appAutosaveHookText.includes('Autosave failed: Storage full') && appAutosaveHookText.includes('prepareIndexedDbAutosaveBase') && appAutosaveHookText.includes('commitIndexedDbAutosaveSnapshot') && autosaveTransactionText.includes('terminate();') && autosaveTransactionText.includes('projectForPersistence(project)') && autosaveWorkerText.includes('serializeProjectCompact') && autosaveWorkerText.includes('autosaveFingerprint(serialized)') && autosaveIndexedDbText.includes('database.transaction(AUTOSAVE_INDEXED_DB_STORE, "readwrite")') && autosaveIndexedDbText.includes('AUTOSAVE_SNAPSHOT_MAX_BYTES') && autosaveIndexedDbText.includes('AUTOSAVE_JOURNAL_MAX_BYTES') && autosaveIndexedDbText.includes('result = await backend.commit(plan, false)') && projectSerializationText.includes('lastExport: _lastExport') && projectSerializationText.includes('JSON.stringify(versionedProject(project))') && autosaveFingerprintText.includes('UTF-8 byte count without allocating') && projectAutosaveFormatText.includes('retainedGenerations: 1 | 2') && workflowSpecText.includes('Autosave keeps cold boot idle and bounds 1/3/5MB writes at 6x CPU') && workflowSpecText.includes('Autosave quota failure is visible in the status dock'), 'autosave strips transient export bytes before structured clone, waits for an explicit project choice or authored edit, prepares compact measured bytes in a disposable worker, atomically commits IndexedDB generations under snapshot/journal budgets, reports failure, and drops the optional recovery generation under quota pressure');
assert(appAutosaveHookText.includes('const allowed = authorized && !options.suspended && project.settings.autosave') && appAutosaveHookText.includes('transaction.setSuspended(!allowed)') && appAutosaveHookText.includes('if (canFlush()) transaction.flush()') && appAutosaveHookText.includes('if (allowedRef.current) transaction.accept(latestProjectRef.current)') && appAutosaveHookText.includes('if (allowedRef.current) transaction.flush()'), 'autosave scheduling, intervals, lifecycle flushes, and disposal all obey the explicit project decision and enabled setting');
assert(autosaveRecoveryWorkerClientText.includes('data.output.mutation && !options.readOnly') && autosaveRecoveryWorkerClientText.indexOf('await options.accept(data.output.result)') >= 0 && autosaveRecoveryWorkerClientText.indexOf('await options.accept(data.output.result)') < autosaveRecoveryWorkerClientText.indexOf('await backend.migrate(') && autosaveRecoveryWorkerClientText.includes('requestState.shouldApply() &&\n                  autosaveRecoveryStorageIsCurrent'), 'cold discovery does not migrate, explicit recovery confirms the validated candidate before mutation, and awaited migration rechecks project authority');
...
assert(gettingStartedDialogCommandSource.includes('Starter rig') && gettingStartedDialogCommandSource.includes('Character file') && gettingStartedDialogCommandSource.includes('Open Project') && gettingStartedDialogCommandSource.includes('onOpenProject') && !gettingStartedDialogCommandSource.includes('importInputRef') && !gettingStartedDialogCommandSource.includes('>Humanoid<') && !gettingStartedDialogCommandSource.includes('Import project'), 'Getting Started keeps compact creation tiles and one prominent shared Open Project action');
...
assert(appUiText.includes('GuidedLessonMotionPreview') && appUiText.includes('guided-project-preview-${lessonId}') && appUiText.includes('data-preview-mode="rendered-character-motion"') && appUiText.includes('data-motion-preview="character-path"') && appUiText.includes('gearTrainCenters(mechanism)') && indexText.includes('repeat(auto-fit, minmax(13rem, 1fr))') && indexText.includes('.guided-project-preview') && indexText.includes('max-height: calc(100dvh - 2rem)') && indexText.includes('min-height: 8.5rem'), 'Guide cards retain real thumbnails while compact starter choices leave Open Project visible');
...
assert(appUiText.includes('Change') && appUiText.includes('Build') && appUiText.includes('data-change-cue') && appUiText.includes('data-direct-translation') && appUiText.includes('data-evidence-cue') && appUiText.includes('data-expected-answer') && appUiText.includes('data-clip-slot') && gettingStartedDialogCommandSource.includes('guided-card-cues') && !gettingStartedDialogCommandSource.includes('starter-card-cues'), 'Guided result cards retain two novice cues and teacher metadata; top creation choices stay compact');
assert(characterLessonOwnershipText.includes('character-make-it-yours') && characterLessonOwnershipText.includes('Make it yours') && characterLessonOwnershipText.includes('data-change-cue={activeClassroomLesson.changeCue}') && characterLessonOwnershipText.includes('data-build-cue={activeClassroomLesson.buildCue}') && characterLessonOwnershipText.includes('Edit parts') && characterLessonOwnershipText.includes('Place joints') && characterLessonOwnershipText.includes('Change path') && characterLessonOwnershipText.includes('Fit mechanism') && characterSelectionText.includes('<CharacterLessonOwnership') && !appText.includes('Change {activeClassroomLesson.changeCue}') && !appText.includes('Build {activeClassroomLesson.buildCue}'), 'Guided lessons land on Character with compact ownership cues for parts, joints, path, and mechanism fit');
...
assert(appControllerSource.includes('useProjectHistory(createEmptyProject)') && appProjectHistoryHookText.includes('return { present: createInitialProject(), past: [], future: [] }'), 'App history initializes empty; candidate discovery never chooses the student project');
const newProjectCommandText = appProjectCommandsHookText.slice(
  appProjectCommandsHookText.indexOf('  const newProject = async () => {'),
  appProjectCommandsHookText.indexOf('  const openClassroomLesson = async ('),
);
assert(newProjectCommandText.includes('const next = createEmptyProject()') && newProjectCommandText.includes('projectDecision.begin()') && newProjectCommandText.includes('confirmReplacement("Start a new project", newProject)') && newProjectCommandText.includes('projectDecision.complete(token)') && newProjectCommandText.includes('projectDecision.isCurrent(token) ? next : current') && newProjectCommandText.includes('resetHistory: true'), 'New Project confirms replacement, authorizes one empty project, resets history, and fences delayed replacement callbacks');
...
assert(readFileSync('runtime/persistence/projectDecisionBoundary.ts', 'utf8').includes('project.sceneObjectOrder.length > 0') && readFileSync('runtime/persistence/projectDecisionBoundary.ts', 'utf8').includes('Object.keys(project.sceneObjects).length > 0'), 'New Project discard confirmation treats Character-created scene objects as user work');
...
assert(!appText.includes('<WelcomeDialog') && !appUiText.includes('WelcomeDialog') && !appText.includes('setShowWelcome') && !appText.includes('setShowGettingStarted(!hideNextTime)') && appControllerSource.includes('useStartupFlow(notes.hasNew)') && appWorkspaceShellText.includes('booting || showGettingStarted || support.startupAnnouncement'), 'Startup coordinates static boot, eligible announcement, and entry without stacking dialogs');
...
assert(characterImportControlsText.includes('character-add-scene-object') && characterImportControlsText.includes('scene-object-image-input') && characterImportControlsText.includes('data-object-image-worker="on-demand"') && characterImportControlsText.includes('accept="image/png,image/jpeg,image/webp,image/svg+xml"') && characterSelectionText.includes('createSceneObjectImageWorkerClient()') && characterSelectionText.includes('objectImageClient.request(file, uid("object")') && characterSelectionText.includes('objectImageClient.dispose()') && characterSelectionText.includes('latestObjectImportProjectRef.current !== source') && sceneObjectImageJobText.includes('contourPoints') && sceneObjectImagePolicyText.includes('Object SVG must be local artwork only.') && sceneObjectImageJobText.includes('originalImageDataUrl') && sceneObjectImageJobText.includes('createPortableProjectBlob') && sceneObjectInspectorText.includes('data-testid="scene-object-inspector"') && sceneObjectInspectorText.includes('Object name') && sceneObjectInspectorText.includes('Size · use Scale') && sceneObjectInspectorText.includes('Delete object'), 'Character imports original object artwork through a captured-source worker and verifies portable capacity before committing it');
...
assert(pathCanvasPaneText.includes('onSelectSceneObject={pathLocked ? undefined : (objectId)') && designFoundryPreviewText.includes('onAutomataSceneObjectSelect={dispatch ? (objectId)') && designFoundryPreviewText.includes('onAutomataPartSelect={dispatch ? (partId)') && designFoundryPreviewText.includes('useWorkingPreviewCamera') && designFoundryPreviewText.includes('{...cameraEvents}') && readFileSync('components/stages/mechanism/useWorkingPreviewCamera.ts', 'utf8').includes('const onWheel: WheelEventHandler') && foundry3dText.includes('onWheel={onWheel}') && foundry3dText.includes('pickAutomataTarget(event)'), 'Path and Mechanism Design retain Character-created object selection while the shared working-camera hook routes viewport input through Foundry');
const sceneObjectUiOwnerFiles = new Set([
  join(process.cwd(), 'components', 'stages', 'character', 'CharacterSelection.tsx'),
  join(process.cwd(), 'components', 'stages', 'character', 'CharacterArtworkWorkspace.tsx'),
...
assert(appText.includes('editCharacterParts') && appCharacterImportActionsHookText.includes('const editCharacterParts =') && appCharacterImportActionsHookText.includes('setStage("character")'), 'Character edit controls route through the shared import/action hook to the functional Character tab');
assert(partInspectorText.includes('className={`compact-number') && partInspectorText.includes('<summary>Artwork</summary>') && partInspectorText.includes('!part.artwork &&') && partInspectorText.includes('Art width') && partInspectorText.includes('Art offset X'), 'Character part inspector keeps transforms compact and hides legacy artwork extents once retained painting defines its own stable frame');
...
assert(partInspectorText.includes('data-testid="part-cut-controls"') && cutOutlineEditorText.includes('data-testid="shape-editor"') && partInspectorText.includes('Change shape') && !partInspectorText.includes('Cut point X'), 'Character part inspector opens the shared canvas shape editor');
assert(partInspectorText.includes('sourceTextureUrl={sourceTextureUrl}') && cutOutlineEditorText.includes('sourceImageFrame') && cutOutlineEditorText.includes('data-testid="shape-source-image"') && cutOutlineEditorText.includes('Source guide'), 'imported character outlines retain an optional full-source context guide');
assert(partInspectorText.includes('contourSource: "user"') && partInspectorText.includes('artwork: artworkForOwner(part)') && cutOutlineEditorText.includes('Add point') && cutOutlineEditorText.includes('Use shape'), 'a deliberate shape commit preserves the original art frame and editable user contour');
assert(cutOutlineEditorText.includes('Draw outline') && cutOutlineEditorText.includes('rectangleOutline') && cutOutlineEditorText.includes('ellipseOutline') && cutOutlineEditorText.includes('validatePhysicalOutline(candidate, { attachments })') && characterSetupPanelText.includes('project={partPanelProject}'), 'shape drawing and presets share current physical attachment validation');
assert(cutOutlineEditorText.includes('setPointerCapture') && cutOutlineEditorText.includes('onPointerCancel={cancelGesture}') && !partInspectorText.includes('commitCut(editableCutPoints)') && !existsSync(join(process.cwd(), 'components', 'stages', 'character', 'CutOutlineEditorDialog.tsx')), 'opening and canceled gestures leave project geometry unchanged and the immediate-write legacy editor is removed');
...
assert(blueprintCanvasBlock.includes('<BlueprintBuildPreview') && blueprintCanvasBlock.includes('buildPlan={buildPlan}') && blueprintCanvasBlock.includes('selectedMechanism={selectedBuildMechanism}'), 'Blueprint center canvas renders the canonical BuildPlan geometry directly');
...
assert(blueprintCanvasBlock.includes('data-visual-level="canonical-build-plan"') && !blueprintControlPanelText.includes('blueprint-more-exports') && !blueprintControlPanelText.includes('Assembly guide') && !blueprintControlPanelText.includes('Teacher files'), 'Blueprint keeps canonical physical build geometry central and removes teacher/assembly exports from the primary workflow');
assert(blueprintExportText.includes('<BlueprintControlPanel') && blueprintControlPanelText.includes('data-testid="blueprint-control-panel"') && blueprintControlPanelText.includes('data-testid="blueprint-build-print"') && blueprintExportText.includes('<BlueprintDetailPanel') && blueprintDetailPanelText.includes('data-testid="blueprint-stack-summary"'), 'Blueprint workflow controls/downloads and Blueprint recipe detail live outside the stage wrapper behind tested panel seams');
assert(blueprintExportText.includes('createBlueprintPackageWorkerClient()') && !blueprintExportText.includes('createBlueprintPackage(project)') && blueprintControlPanelText.includes('data-blueprint-package-worker="on-demand"') && blueprintControlPanelText.includes('packageStatus === "running" ? "Cancel" : "Download Build PDF"') && blueprintExportText.includes('fabricationPackage.buildPacketPdf') && blueprintExportText.includes('downloadPdf('), 'Blueprint exposes one direct, cancelable, binary-safe complete Build PDF action');
assert(blueprintControlPanelText.includes('data-testid="blueprint-character-template"') && blueprintExportText.includes('requestCharacterTemplate(project') && !blueprintControlPanelText.includes('Blueprint first.'), 'Clean character outlines remain independently downloadable from a character-only BuildPlan');
...
assert(blueprintExportText.includes('currentPackage && exposePackageDiagnostics') && blueprintExportText.includes('__MOTIONSMITH_E2E_DIAGNOSTICS__') && blueprintExportText.includes('__MOTIONSMITH_CHROMEBOOK_AUDIT__') && blueprintExportText.includes('JSON.stringify(currentPackage)'), 'multi-megabyte package JSON is exposed only in ordinary diagnostics builds, not production or memory-audit DOM');
...
assert(blueprintCanvasBlock.includes('<BlueprintBuildPreview') && !blueprintCanvasBlock.includes('<DeferredThreePuppetPreview'), 'Blueprint center canvas renders the canonical physical BuildPlan drawing instead of a mutable project scene');
...
assert(assemblyCanvasPaneText.includes('<AssemblyCharacterThreePreview') && assemblyCanvasPaneText.includes('<AssemblyMechanismThreePreview') && assemblyThreePreviewText.includes('ThreeFoundryPreview') && assemblyThreePreviewText.includes('if (!previewModel || !mechanism)') && assemblyThreePreviewText.includes('<AssemblyLocalPartsPreview') && assemblyLocalPartsPreviewText.includes('<DeferredThreePuppetPreview') && assemblyLocalPartsPreviewText.includes('project={project}') && assemblyLocalPartsPreviewText.includes('mechanisms: false') && !assemblyLocalPartsPreviewText.includes('createDefaultMechanism'), 'Assembly uses shared Foundry for mechanism builds and the existing physical Puppet scene for unpowered painted pieces without inventing a mechanism');
...
assert(assemblyGuideModelText.includes('export const prepareAssemblyGuideModel') && assemblyGuideModelText.includes('export const selectAssemblyGuideStep') && assemblyGuideModelText.includes('export const buildAssemblyGuideModel') && assemblyGuideModelText.includes('pendingRecipeForMechanism') && assemblyGuideModelText.includes('createBuildPlanV1') && assemblyGuideModelText.includes('characterAssemblyPlanFromBuildPlan') && assemblyGuideModelText.includes('resetKey: `${activeAssemblyMode}:${selectedRecipe?.mechanismId ?? "none"}:${lane}`') && !assemblyGuideModelText.includes('useState') && !assemblyGuideModelText.includes('window.') && !assemblyGuideModelText.includes('document.') && !assemblyGuideModelText.includes('dispatch('), 'Assembly guide model helper derives screen steps from BuildPlan and separates static preparation from pure current-step selection');
assert(assemblyBlock.includes('const preparedModel = useMemo(') && assemblyBlock.includes('selectAssemblyGuideStep(preparedModel, stepIndex)') && assemblyBlock.includes('projectContentFingerprint(project)') && assemblyBlock.includes('buildPlanSourceDigest(project, "complete", lane)') && !assemblyBlock.includes('createBlueprintPackageWorkerClient') && !assemblyBlock.includes('downloadText(') && !assemblyControlPanelText.includes('Download Blueprint PDF') && !assemblyControlPanelText.includes('Download Character PDF'), 'Assembly memoizes its semantic BuildPlan while Blueprint exclusively owns packet and template downloads');
...
assert.deepEqual(fabricablePartOutlinePoints(invalidContourPart, oversizedLocalJoints), [], 'invalid explicit user contours cannot silently become a capsule');
const invalidFallbackBounds = partOutlineBounds(fabricablePartOutlinePoints({ ...invalidContourPart, contourSource: 'imported' }, oversizedLocalJoints));
assert(invalidFallbackBounds.width > 20 && invalidFallbackBounds.height > 100, 'legacy degenerate imported contours preserve the joint-chain fallback');
...
assert(productionArmLandmarks.every(point => pointInsideOutline(point, productionArmOutline, 0.5)), 'generated arm silhouette contains its guide landmarks; drilling follows the separate canonical pin plan');
...
assert.equal(describeMotionChain(sample, 'right_arm_lower', 'right_elbow').kind, 'two-joint-direct', 'moving the elbow defaults to its attached shoulder');
assert.equal(describeMotionChain(sample, 'right_arm_lower', 'right_hand').kind, 'three-joint-ik', 'a lower-arm hand handle defaults to the whole attached arm');
...
assert(directPinnedHand && Math.hypot(directPinnedHand.x - 210, directPinnedHand.y - 40) > 1, 'unreachable mechanism outputs cannot stretch a connected arm');
assert(directPinnedPreview.warnings?.right_arm_lower?.includes('Move target within reach'), 'an unreachable physical binding reports the correction');
...
assert(directPreviewHand && Math.hypot(directPreviewHand.x - 210, directPreviewHand.y - 40) > 1, 'path preview preserves the same attached limb reach as mechanism preview');
...
const secondFourBarPath = {
  ...
  id: 'fabrication-fit-left-arm-path',
  partId: 'left_arm_lower',
  targetAnchorJointId: 'left_hand',
  chainRootJointId: 'left_shoulder',
  points: fabricationSample.paths['fabrication-fit-path'].points.map(point => ({ x: point.x - 320, y: point.y })),
};
const secondFourBarFit = fitMechanismToTargetPath(
  {
    ...
    mechanisms: [],
    paths: {
      ...Object.fromEntries(Object.entries(fabricationSample.paths).map(([id, path]) =>
        [id, { ...path, enabled: false }],
      )),
      [secondFourBarPath.id]: secondFourBarPath,
    },
  },
  {
...
const canonicalPrintHoles = characterFabricationHoles(twoFourBars);
...
assert.deepEqual(canonicalPrintHoles.get('head')?.map(hole => hole.jointId), ['neck'], 'the head cut piece drills its assembly neck pin, never its head-top guide');
characterPrintLayout.parts.forEach(item => {
  const physicalHoles = canonicalPrintHoles.get(item.part.id) ?? [];
  const topology = preparePuppetPartTopology(item.part, twoFourBars.skeleton, resolveRenderPerformancePolicy('balanced').partTopology, physicalHoles);
  assert.deepEqual(topology.localHoles, physicalHoles.map(hole => hole.center), `${item.part.id} Three holes are the canonical pin list`);
  assert.equal(item.holeMm.length, physicalHoles.length, `${item.part.id} print and physical preview drill the same hole count`);
  const sceneOrigin = ownerLocalToScene(topology.outline[0], item.part.transform);
  const offset = { x: item.outlineMm[0].x - sceneOrigin.x / SCENE_PX_PER_MM,
    y: item.outlineMm[0].y + sceneOrigin.y / SCENE_PX_PER_MM };
  physicalHoles.forEach((hole, index) => {
    const sceneHole = ownerLocalToScene(hole.center, item.part.transform);
    assert(Math.hypot(item.holeMm[index].x - (offset.x + sceneHole.x / SCENE_PX_PER_MM),
      item.holeMm[index].y - (offset.y - sceneHole.y / SCENE_PX_PER_MM)) < 1e-7,
      `${item.part.id} print hole shares the outline's local-to-page registration`);
    assert(Math.abs(hole.radius * item.part.transform.scale / SCENE_PX_PER_MM - characterPrintLayout.holeRadiusMm) < 1e-9,
      `${item.part.id} preview radius matches its physical print radius`);
  });
});
...
assert(pkg.blueprintPdf?.startsWith('%PDF-') && pkg.blueprintPdf.includes('Blueprint') && pkg.cutSheetPdf === '' && pkg.buildPacketPdf === undefined, 'fabrication package carries one canonical true-scale Blueprint PDF without duplicate legacy payloads');
assert.equal(directMakeCutSheetPdf(twoFourBars, pkg.recipes), pkg.blueprintPdf, 'fabrication package Blueprint PDF matches the direct canonical generator');
...
assert(pkg.customPartsSvg.includes(`width="${twoFourBars.settings.physicalKit.sheetWidthMm}mm"`) && pkg.customPartsSvg.includes(`data-character-print-page-count="${characterPrintLayout.pageCount}"`), 'character custom parts SVG is paginated on Letter without silent scaling');
...
assert(pkg.customPartsSvg.includes('data-character-exploded-sheet="true"'), 'character custom parts SVG groups all parts on one XML-safe exploded sheet');
...
assert(pkg.customPartsPdf.startsWith('%PDF-') && pkg.customPartsPdf.includes('Print at 100% scale') && pkg.customPartsPdf.includes('/MediaBox [0 0 612 792]'), 'fabrication package includes a real-size Letter character PDF with an actionable scale label');
assert(!pkg.customPartsPdf.includes('character-sheet-page-count') && !pkg.customPartsPdf.includes('(Character plan '), 'print headers exclude internal diagnostic keys and plan hashes');
...
const baseMechanismParts = mechanismRequiredParts(twoFourBars.mechanisms[0]);
assert(baseMechanismParts.every(required => {
  const actual = pkg.recipes[0].requiredParts.find(part => part.name === required.name);
  return actual && actual.quantity >= required.quantity;
}), 'recipe required parts include every mechanism metadata default');
assert(pkg.recipes[0].requiredParts.some(part => part.name === 'S10 spacer' && part.quantity === 9) && pkg.recipes[0].requiredParts.some(part => part.name === 'Paper fastener' && part.quantity === 1), 'recipe required parts add the physical output attachment spacer and fastener');
...
const persistedExportedProject = deserializePortableProject(exportedProject);
...
assert.deepEqual(persistedExportedProject.mechanisms, JSON.parse(JSON.stringify(loadProjectSnapshot({ ...exportedProject, lastExport: undefined }).mechanisms)), 'omitting transient export files preserves canonical JSON mechanism state including migrated output bindings');
...
const svgTextureSceneObject = loadProjectSnapshot({
  ...deserializePortableProject(movedSceneObject),
  sceneObjects: {
    'object-piggy': {
      ...sceneObject,
...
assert.equal(svgTextureSceneObject.sceneObjects['object-piggy'].textureUrl, 'data:image/svg+xml;base64,PHN2Zz48L3N2Zz4=', 'scene object snapshots preserve original self-contained SVG bytes');
const unsafeSvgTextureSceneObject = loadProjectSnapshot({
  ...svgTextureSceneObject,
  sceneObjects: { 'object-piggy': { ...sceneObject,
    textureUrl: `data:image/svg+xml;base64,${Buffer.from('<svg width="1024" height="768"><image href="https://example.com/unvalidated.png"/></svg>').toString('base64')}`,
  } },
});
assert.equal(unsafeSvgTextureSceneObject.sceneObjects['object-piggy'].textureUrl, undefined, 'original SVG retention still rejects external or unbudgeted embedded image resources');
const boundedSvgTexture = 'data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iNTEyIiBoZWlnaHQ9IjUxMiI+PC9zdmc+';
const boundedSvgTextureSceneObject = loadProjectSnapshot({
  ...deserializePortableProject(movedSceneObject),
  sceneObjects: {
    'object-piggy': {
      ...sceneObject,
      textureUrl: boundedSvgTexture,
      contourPoints: sceneObject.contourPoints
    }
  },
  sceneObjectOrder: ['object-piggy']
});
assert.equal(boundedSvgTextureSceneObject.sceneObjects['object-piggy'].textureUrl, boundedSvgTexture, 'scene object snapshots preserve legacy worker-bounded local SVG texture data');
...
assert(!validateForFabrication(noEnabledMechanismProject).errors.some(e => e.includes('No enabled mechanism')), 'students can print flat pieces before adding a mechanism');
assert.equal(createFabricationPackage(noEnabledMechanismProject).recipes.length, 0, 'flat-piece-only output never invents a mechanism recipe');
...
for (const phase of [0, Math.PI / 2, Math.PI, Math.PI * 1.5]) {
  ...
  const shoulder = fabricationSample.skeleton!.joints.right_shoulder.position;
  const elbow = fabricationSample.skeleton!.joints.right_elbow.position;
  const hand = fabricationSample.skeleton!.joints.right_hand.position;
  const reach = distance(shoulder, elbow) + distance(elbow, hand);
  if (distance(shoulder, physicalTarget) <= reach) {
    assert(distance(targetJoint!, physicalTarget) < 1e-9, 'reachable mechanism outputs remain attached throughout scrubbing');
  } else {
    assert(preview.warnings?.[sampleMechanism.id]?.includes('Move target within reach'), 'an unreachable fixture output is reported instead of stretching the character');
  }
  assert(Math.abs(distance(preview.skeleton!.joints.right_elbow.position, targetJoint!) - distance(elbow, hand)) < 1e-6, 'every scrub sample preserves the lower-arm length');
}
...
assert(validateForFabrication(conflictProject).errors.some(e => e.includes('only one mechanism can own a target')), 'blueprint export blocks ambiguous duplicate target drivers');
