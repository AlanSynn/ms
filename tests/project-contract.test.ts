...
assert(normalizedCodebaseCleanupPlan.includes('`App.tsx` | 1284') && normalizedCodebaseCleanupPlan.includes('Command keyboard binding, boot/cache lifecycle, autosave/workspace persistence, and Character/Path/Foundry/Options/Assembly/Mechanism Design stage seams are extracted') && normalizedCodebaseCleanupPlan.includes('`hooks/useAppOnnxBootstrap.ts` | 88') && normalizedCodebaseCleanupPlan.includes('`hooks/useProjectAutosave.ts` | 23') && normalizedCodebaseCleanupPlan.includes('`utils/projectPersistence.ts` | 158'), 'cleanup plan records the current App.tsx hotspot and completed command/persistence/stage seams');
