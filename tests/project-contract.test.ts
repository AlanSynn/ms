...
assert(ciWorkflowText.includes('PLAYWRIGHT_WORKERS: 3') && ciWorkflowText.includes('tests/browser/webgl-recovery.spec.ts'), 'pull-request browser coverage is focused and uses bounded parallel workers');
...
assert(performanceWorkflowText.includes('workflow_dispatch:') && performanceWorkflowText.includes('schedule:'), 'performance audits are available manually and on a nightly schedule');
assert(performanceWorkflowText.includes('bun run test:chromebook-audit') && performanceWorkflowText.includes('bun run test:chromebook-audit:full') && !performanceWorkflowText.includes('real-ai'), 'performance workflow keeps short and full non-AI scopes explicit');
...
assert(automataSceneModelText.includes('createFoundryMechanismPreviewRuntime') && designFoundryPreviewText.includes('createAutomataSceneRuntime') && designFoundryPreviewText.includes('sampleAutomataSceneRuntime') && assemblyThreePreviewText.includes('createAutomataSceneRuntime') && assemblyThreePreviewText.includes('sampleAutomataSceneRuntime') && foundryPreviewModelText.includes('createFoundryPlaybackFrame') && foundryPreviewModelText.includes('generateFoundryPlaybackPointTraces') && !designFoundryPreviewText.includes('generateMechanismPointTraces') && !assemblyThreePreviewText.includes('generateMechanismPointTraces') && !assemblyThreePreviewText.includes('fitMechanismSimulationWithContext'), 'Mechanism Design and Assembly retain phase-invariant Foundry state and sample motion without private legacy simulation paths');
...
const assertMotionFitSourceContracts = () => {
  ...
  const assemblyFittedMechanismContinuity =
    ...
    assemblyThreePreviewText.includes('sampleAutomataSceneRuntime') &&
...
assert(projectImportPolicyText.includes('projectBytes: 24 * MEBIBYTE') && projectImportPolicyText.includes('packageFileCount: 128') && projectImportPolicyText.includes('packageAssetBytes: 6 * MEBIBYTE') && projectImportPolicyText.includes('packageTotalAssetBytes: 24 * MEBIBYTE') && projectImportPolicyText.includes('packageTotalBytes: 32 * MEBIBYTE'), 'classroom project and character imports have explicit per-file, file-count, image-total, and package-total memory bounds');
...
assert(appText.includes('useProjectAutosave(project') && appControllerSource.includes('readAutosaveProject(initialProject)') && appProjectCommandsHookText.includes('readAutosaveProject') && appProjectCommandsHookText.includes('readWorkspaceLayoutSnapshot') && appProjectCommandsHookText.includes('writeWorkspaceLayoutSnapshot') && appAutosaveHookText.includes('createAutosaveTransaction') && appAutosaveHookText.includes('pagehide') && appAutosaveHookText.includes('beforeunload') && projectPersistenceText.includes('readStorageWithLegacy') && projectPersistenceText.includes('migrateStorageValue') && projectAutosaveFormatText.includes('motionsmith.autosave'), 'MotionSmith storage rename keeps legacy autosave/workspace migration hooks behind the persistence seam and reload protection');
assert(appAutosaveHookText.includes('project === initialProjectRef.current') && appAutosaveHookText.includes('Autosave failed: Storage full') && autosaveTransactionText.includes('terminate();') && autosaveWorkerText.includes('serializeProjectCompact') && autosaveWorkerText.includes('autosaveFingerprint(serialized)') && projectSerializationText.includes('JSON.stringify(versionedProject(project))') && autosaveFingerprintText.includes('UTF-8 byte count without allocating') && projectAutosaveTransactionsText.includes('readAutosaveJournalSnapshot(target, false)') && projectAutosaveTransactionsText.includes('return write(false)') && projectAutosaveFormatText.includes('retainedGenerations: 1 | 2') && workflowSpecText.includes('Autosave keeps cold boot idle and bounds 1/3/5MB writes at 6x CPU') && workflowSpecText.includes('Autosave quota failure is visible in the status dock'), 'autosave stays off the cold boot path, prepares compact measured bytes in a disposable worker, avoids rescanning committed payloads, reports failure, and drops only the optional recovery generation under quota pressure');
...
assert(gettingStartedDialogCommandSource.includes('getting-started-card-humanoid') && gettingStartedDialogCommandSource.includes('getting-started-card-character') && gettingStartedDialogCommandSource.includes('getting-started-package-input') && !gettingStartedDialogCommandSource.includes('getting-started-card-image') && !appWorkspaceShellText.includes('showStarterImages'), 'Getting Started exposes local package import without image-recognition starter surfaces');
...
assert(blueprintCanvasBlock.includes('<ThreePuppetPreview') && blueprintCanvasBlock.includes('testId="blueprint-three-puppet"') && blueprintCanvasBlock.includes('initialCameraPreset="front"'), 'Blueprint center canvas previews the canonical components in the shared front-view Three scene');
...
assert(blueprintPackageWorkerClientText.includes('new URL("../../workers/blueprintPackageWorker.ts", import.meta.url)') && blueprintPackageWorkerClientText.includes('active.firstFrame') && blueprintPackageWorkerClientText.includes('active.secondFrame') && blueprintPackageWorkerClientText.includes('lastExport: undefined') && blueprintPackageWorkerClientText.includes('worker.terminate()') && blueprintPackageWorkerText.includes('await import('), 'Blueprint package work waits for two paints, omits stale generated files from worker input, lazy-loads generation code, and releases ownership');
assert(blueprintExportText.includes('pkg && __MOTIONSMITH_E2E_DIAGNOSTICS__') && blueprintExportText.includes('JSON.stringify(pkg)'), 'multi-megabyte package JSON is exposed only in diagnostics builds, not serialized into hidden production DOM');
assert(fabricationCustomPartsText.includes('const boundaryColumns = new Set(') && fabricationCustomPartsText.includes('Math.max(1.5, Math.min(2.5, holeRadiusMm))') && b695BlueprintTestText.includes('compacted classroom STL has no open mesh edges') && b695BlueprintTestText.includes('2_500_000'), 'custom-parts STL uses a bounded conforming strip mesh with a locked classroom byte budget');
...
assert(blueprintCanvasBlock.includes('<ThreePuppetPreview') && !blueprintCanvasBlock.includes('<Canvas project={project}'), 'Blueprint center canvas reuses the animated 3D/2.5D workbench scene instead of a separate SVG document');
...
assert(assemblyThreePreviewText.includes('sampleAutomataSceneRuntime') && assemblyThreePreviewText.includes('automataContext={automataContext}') && automataSceneModelText.includes('createFoundryMechanismPreviewRuntime') && !assemblyThreePreviewText.includes('mechanisms={[]}'), 'Assembly character context consumes the retained shared automata/Foundry preview seam instead of private puppet mechanism fallbacks');
...
assert(assemblyBlock.includes('const preparedModel = useMemo(') && assemblyBlock.includes('selectAssemblyGuideStep(preparedModel, stepIndex)') && assemblyBlock.includes('useMemo(() => validateForFabrication(project), [project])') && assemblyBlock.includes('createFabricationPackage(project)') && assemblyBlock.includes('window.open') && assemblyBlock.includes('downloadText'), 'Assembly Guide memoizes validation and static recipes/steps independently from the changing playback step');
