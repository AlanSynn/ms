...
assert.equal(packageJson.scripts['test:contracts'], 'bun tests/project-contract.test.ts && bun tests/b695-fit.test.ts && bun tests/four-bar-fit-retention.test.ts && bun tests/mechanism-recommendation-worker.test.ts && bun tests/mechanism-optimizer-worker.test.ts && bun tests/web-onnx-inference-worker.test.ts && bun tests/web-onnx-image-worker.test.ts && bun tests/character-import-supersession.test.ts && bun tests/tracking-media-policy.test.ts && bun tests/chromebook-audit-contract.test.ts && bun tests/render-performance-policy.test.ts && bun tests/interactive-sampling.test.ts && bun tests/automata-scene-runtime.test.ts && bun tests/three-resource-retention.test.ts && bun tests/cadenced-playback-sampler.test.ts', 'contract tests run directly through Bun without noisy temporary bundling and include deterministic fit, bounded retention, heavy-worker, bounded-image, import-supersession, media-memory, Chromebook, render-policy, interaction sampling, golden-runtime, resource-retention, and cadenced-sampling gates');
