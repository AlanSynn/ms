...
assert(fabricationCustomPartsText.includes('const boundaryColumns = new Set(') && fabricationCustomPartsText.includes('Math.max(1.5, Math.min(2.75, holeRadiusMm * 1.375))') && b695BlueprintTestText.includes('compacted classroom STL has no open mesh edges') && b695BlueprintTestText.includes('1_300_000'), 'custom-parts STL uses a bounded conforming strip mesh with a locked classroom byte budget');
