(() => {
  const run = async () => {
    ...
    if (!rig || !toolbar) throw new Error('Foundry playback probe targets are unavailable');
    const button = (label) =>
  ...
  window.__motionsmithAbPlaybackResult = null;
  run().then(
    (result) => {
      window.__motionsmithAbPlaybackResult = result;
    },
    (error) => {
      window.__motionsmithAbPlaybackResult = {
