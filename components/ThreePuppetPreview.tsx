...
const EMPTY_ANIMATED_PARTS: Record<string, BodyPartLayer> = {};
const EMPTY_ANIMATED_SCENE_OBJECTS: Record<string, SceneObject> = {};
...
const pruneSharedGeometryCache = (idleRetentionLimit = 0) => {
  ...
  pruneUnusedThreeResourceCache(
    sharedGeometryCache,
    used,
    Math.max(configuredLimit, idleRetentionLimit),
  );
};
...
const puppetMechanismInventory = (mechanism: MechanismConfig): MechanismInventory => {
  const fallback = ({
    ...
    gear_linkage: { parts: Math.max(7, gearTrainPitchRadii(mechanism).length + 5), holes: Math.max(22, gearTrainPitchRadii(mechanism).length * 8 + 6), slots: 0, gears: gearTrainPitchRadii(mechanism).length, racks: 0, cams: 0, followers: 0, endStops: 0 },
    planetary_gear: { parts: 7, holes: 18, slots: 0, gears: 3, racks: 0, cams: 0, followers: 0, endStops: 0 }
  }[mechanism.type]);
  const referenceHoleCount = referenceRequiredPartsHoleCount(mechanismRequiredParts(mechanism));
...
export const ThreePuppetPreview = ({ project, animatedParts = EMPTY_ANIMATED_PARTS, animatedSceneObjects = EMPTY_ANIMATED_SCENE_OBJECTS, skeleton, mechanisms, paths, selectedPathId, selectedPathPointIndex, angle = 0, playback, viewport, setViewport, inputMode = 'always', testId = 'three-puppet', cameraPresets = PUPPET_CAMERA_PRESETS, showToolbar = true, showCameraPresets = true, initialCameraPreset, initialLayers, assemblyOverlay, drawMode = false, onDrawPoint, onDrawEnd, onSelectPathPoint, onMovePathPoint, onEndPathPointEdit, pathGestureDraft, onSelectPart, onSelectSceneObject, onSelectMechanism, onSelectJoint, onSelectOnlyPointerDown, onSelectOnlyPointerMove, onSelectOnlyPointerUp, onSelectOnlyPointerCancel, onSelectOnlyWheel }: {
  ...
  const jointHardwareRef = useRef<PuppetJointHardwareInstances | null>(null);
  ...
  useEffect(() => {
    ...
    setCameraOrbit(cameraOrbitFromPreset(initialCameraPreset));
  }, [initialCameraPreset]);
  useEffect(() => {
  ...
  renderedPartsRef.current = parts;
  selectedPartIdRef.current = project?.selectedPartId;
  assemblyExplodeAmountRef.current = assemblyExplodeAmount;
  ...
  const holeRingDrawObjectCount = preparedPartTopologies.reduce(
    (sum, topology) => sum + (topology.localHoles.length > 0 ? 1 : 0),
    0,
  );
  ...
  const estimatedObjectCount = 2 + geometryParts.length * 4 + sceneObjects.length * 4 + holeRingDrawObjectCount + (joints.length ? 2 : 0) + bones.length + pathsToRender.length * 4 + mechanismLinkCount * 2 + mechanismsToRender.length * 8 + mechanismInventory.holes + mechanismInventory.gears * 2;
  ...
  useEffect(() => {
    ...
    const gridLines = boardGridLines(kit);
    const gridGeometry = new THREE.BufferGeometry().setFromPoints(
      gridLines.flatMap((line) => [to3(line.a, -0.16), to3(line.b, -0.16)]),
    );
    const grid = new THREE.LineSegments(gridGeometry, materials.grid);
    grid.name = 'board-grid-line-segments';
    roots.staticLayer.add(grid);
    if (E2E_DIAGNOSTICS && stateRef.current) {
      stateRef.current.dataset.threeGridDrawObjectCount = '1';
      stateRef.current.dataset.threeGridSegmentCount = String(gridLines.length);
    }
    render();
  ...
  useEffect(() => {
    ...
    if (joints.length) {
      const hardware = createPuppetJointHardwareInstances({
        jointIds: joints.map((joint) => joint.id),
        pinGeometry: cachedGeometry(
          'skeleton-pin:0.08:0.36:24',
          () => new THREE.CylinderGeometry(0.08, 0.08, 0.36, 24),
        ),
        washerGeometry: cachedGeometry(
          'skeleton-washer:0.12:0.018:8:24',
          () => new THREE.TorusGeometry(0.12, 0.018, 8, 24),
        ),
        pinMaterial: materials.pin,
        washerMaterial: materials.joint,
      });
      roots.skeletonLayer.add(hardware.pins, hardware.washers);
      jointHardwareRef.current = hardware;
    }
    render();
  ...
  useEffect(() => {
    ...
    const jointHardware = jointHardwareRef.current;
    if (jointHardware) {
      updatePuppetJointHardwareInstances({
        ...
        pinZ: 0.35,
        washerZ: 0.55,
        isVisible: (jointId) => !assemblyOverlay || (
          assemblyPhase !== 'character-parts' && assemblyActiveJoints.has(jointId)
        ),
      });
    }
    render();
  ...
  useEffect(() => {
    ...
    renderedMechanisms.forEach(mechanism => {
      ...
      const boardSideSpacerZ = (movingZ: number) =>
        Number((movingZ - (FABRICATION_RENDER_PART_DEPTH + FABRICATION_RENDER_MIN_CLEARANCE) / 2).toFixed(3));
      const uniqueSortedZ = (values: number[]) =>
        values
          .filter(value => Number.isFinite(value))
          .sort((a, b) => a - b)
          .filter((value, index, sorted) => index === 0 || Math.abs(value - sorted[index - 1]) > 0.001);
      const betweenMovingZ = (values: number[], count = 1) => {
        const zValues = uniqueSortedZ(values);
        if (zValues.length < 2) return [];
        const minZ = zValues[0];
        const maxZ = zValues.at(-1) ?? minZ;
        return Array.from({ length: Math.max(1, count) }, (_, index) =>
          Number((minZ + ((maxZ - minZ) * (index + 1)) / (count + 1)).toFixed(3))
        );
      };
      const boardToMovingZ = (movingZ: number) =>
        uniqueSortedZ([zBackClip, boardSideSpacerZ(movingZ), movingZ]);
      const adjacentMovingZ = (a: number, b: number, spacerCount = 1) =>
        uniqueSortedZ([a, ...betweenMovingZ([a, b], spacerCount), b]);
      const pinSpanForZ = (values: number[]) => {
        const zValues = uniqueSortedZ(values);
        const minZ = zValues[0] ?? zBackClip;
        const maxZ = zValues.at(-1) ?? minZ;
        const bottom = minZ - FABRICATION_RENDER_PART_DEPTH / 2 - 0.08;
        const top = maxZ + FABRICATION_RENDER_PART_DEPTH / 2 + 0.18;
        return {
          center: (bottom + top) / 2,
          length: Math.max(0.36, top - bottom)
        };
      };
      const pinSite = (point: Point | undefined, zValues: number[]) => ({ point, zValues: uniqueSortedZ(zValues) });
      ...
      const genericPinSites = () => [
        pinSite(state.p1, boardToMovingZ(zDriver)),
        pinSite(state.p2, boardToMovingZ(zOutput)),
        pinSite(state.j1, adjacentMovingZ(zDriver, zCoupler)),
        pinSite(state.j2, adjacentMovingZ(zCoupler, zOutput)),
        pinSite(state.aux, adjacentMovingZ(zOutput, zDyad)),
        pinSite(state.effector, [zOutput])
      ];
      const pinSites = mechanism.type === 'gear'
        ? gearPinCenters.map(center => pinSite(center, boardToMovingZ(zDriverGear)))
        : mechanism.type === 'gear_linkage'
          ? [
              ...gearPinCenters.map(center => pinSite(center, boardToMovingZ(zDriverGear))),
              pinSite(state.j1, adjacentMovingZ(zDriverGear, zDriver)),
              pinSite(state.j2, adjacentMovingZ(zDriverGear, zOutput, 2)),
              pinSite(state.effector, adjacentMovingZ(zDriver, zOutput))
            ]
          : mechanism.type === 'planetary_gear'
            ? [
                pinSite(state.p1, uniqueSortedZ([zRingGear, zDriverGear, zDriver])),
                pinSite(planetCentersForPins[0], uniqueSortedZ([zRingGear, zOutputMoving, zDriver]))
              ]
            : genericPinSites();
      ...
      if (mechanism.type === 'piston') {
        ...
        updateLink(visual.links.base, undefined, undefined);
        updateLink(visual.links.driver, state.isValid ? state.j1 : undefined, state.isValid ? state.effector : undefined, zDriver);
        updateLink(visual.links.coupler, undefined, undefined);
        updateLink(visual.links.output, state.isValid ? state.j2 : undefined, state.isValid ? state.effector : undefined, zOutput);
        updateLink(visual.links.effector, undefined, undefined);
      ...
      visual.pins.forEach((pin, index) => {
        ...
        if (!point || !site.zValues.length) {
          pin.visible = false;
          return;
        }
        const pinSpan = pinSpanForZ(site.zValues);
        const p = to3(point, pinSpan.center);
        pin.visible = true;
        pin.position.copy(p);
        pin.scale.set(1, pinSpan.length / 0.36, 1);
      });
    });
    render();
  ...
  useEffect(() => {
    ...
    if (!playbackClock || rendererStatus !== 'webgl') return;
    const applyPreview = (preview: MotionPreview | undefined) => {
      ...
      if (previewSkeleton) {
        ...
        const jointHardware = jointHardwareRef.current;
        if (jointHardware) {
          const assemblyActiveJoints = new Set(
            assemblyOverlay?.activeJointIds ?? [],
          );
          updatePuppetJointHardwareInstances({
            hardware: jointHardware,
            joints: Object.values(previewSkeleton.joints),
            viewScale: VIEW_SCALE,
            pinZ: 0.35,
            washerZ: 0.55,
            isVisible: (jointId) => !assemblyOverlay || (
              assemblyPhase !== 'character-parts' &&
              assemblyActiveJoints.has(jointId)
            ),
          });
        }
      }
  ...
  const scenePointAtPointer = (event: Pick<React.PointerEvent<HTMLDivElement>, 'clientX' | 'clientY'>) => {
    const renderer = rendererRef.current;
    const camera = cameraRef.current;
    const roots = rootsRef.current;
    if (!renderer || !camera || !roots) return null;
    const rect = renderer.domElement.getBoundingClientRect();
    ...
    const worldPoint = raycaster.ray.origin.clone().addScaledVector(raycaster.ray.direction, distance);
    const rootPosition = new THREE.Vector3();
    roots.root.getWorldPosition(rootPosition);
    return {
      x: (worldPoint.x - rootPosition.x) * VIEW_SCALE,
      y: (worldPoint.y - rootPosition.y) * VIEW_SCALE,
    } satisfies Point;
  };

  const pathPointAtPointer = (event: Pick<React.PointerEvent<HTMLDivElement>, 'clientX' | 'clientY'>) => {
    const renderer = rendererRef.current;
    const camera = cameraRef.current;
    const roots = rootsRef.current;
    if (!renderer || !camera || !roots || !onSelectPathPoint) return null;
    const rect = renderer.domElement.getBoundingClientRect();
    if (rect.width <= 0 || rect.height <= 0) return null;
    const pointer = new THREE.Vector2(
      ((event.clientX - rect.left) / rect.width) * 2 - 1,
    ...
    for (const hit of raycaster.intersectObjects(pointObjects, true)) {
      ...
      while (node) {
        ...
        const pointIndex = typeof node.userData.pathPointIndex === 'number' ? node.userData.pathPointIndex : undefined;
        if (pathId && pointIndex !== undefined) return { pathId, pointIndex };
        node = node.parent;
      }
    }
    return null;
  ...
  const handleViewerPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (event.button === 0 && inputMode !== 'none' && drawMode && onDrawPoint) {
      const point = scenePointAtPointer(event);
      if (!point) return;
      viewerDragRef.current = {
        pointerId: event.pointerId,
        button: event.button,
    ...
    if (event.button === 0 && inputMode !== 'none' && !drawMode && onSelectPathPoint) {
      const pathPoint = pathPointAtPointer(event);
      if (pathPoint) {
        viewerDragRef.current = {
          pointerId: event.pointerId,
          button: event.button,
        ...
        event.preventDefault();
        event.currentTarget.setPointerCapture(event.pointerId);
        onSelectPathPoint(pathPoint.pathId, pathPoint.pointIndex);
        return;
      }
    }
  ...
  const handleViewerPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (start.mode === 'draw') {
      const point = scenePointAtPointer(event);
      if (point) onDrawPoint?.(point);
      event.preventDefault();
      return;
    }
    if (start.mode === 'path-point') {
      const point = scenePointAtPointer(event);
      if (point) onMovePathPoint?.(point);
      event.preventDefault();
      return;
    }
    event.preventDefault();
  ...
  const pickViewerTarget = (event: Pick<React.PointerEvent<HTMLDivElement>, 'clientX' | 'clientY'>) => {
    if (inputMode === 'none' || (!onSelectPart && !onSelectSceneObject && !onSelectMechanism && !onSelectJoint)) return;
    const renderer = rendererRef.current;
    const camera = cameraRef.current;
    const roots = rootsRef.current;
    ...
    raycaster.setFromCamera(pointer, camera);
    if (onSelectJoint) {
      const jointHits = raycaster.intersectObjects([roots.skeletonLayer], true);
      for (const hit of jointHits) {
        const instancedJointId = puppetJointIdForInstance(
          hit.object,
          hit.instanceId,
        );
        if (instancedJointId) {
          onSelectJoint(instancedJointId);
          return;
        }
        let node: THREE.Object3D | null = hit.object;
        while (node) {
          const jointId = typeof node.userData.jointId === 'string' ? node.userData.jointId : undefined;
          if (jointId) {
  ...
  >
    ...
    {E2E_DIAGNOSTICS && <div
      ...
      data-three-cut-hole-ring-draw-object-count={holeRingDrawObjectCount}
      data-three-cut-hole-ring-instance-count={holeCount}
      data-three-skeleton-hardware-draw-object-count={joints.length ? 2 : 0}
      data-three-skeleton-pin-instance-count={joints.length}
      data-three-skeleton-washer-instance-count={joints.length}
