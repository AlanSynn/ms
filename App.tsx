...
const foundryLayerGeometryContract = (type: MechanismType, label: string, renderKind: string) => {
    ...
    if (type === 'gear_linkage') {
        ...
        if (/drive.*L|Drive L|drive.*linkage/i.test(label)) return `${label}:B-pin-to-R`;
        if (/output.*L|Output L|output.*linkage/i.test(label)) return `${label}:C-pin-to-R`;
...
const foundryGroundLinkLayerIndexes = (type: MechanismType, layers: FoundryRenderLayerLike[]) => {
    if (type !== '4bar') return undefined;
    const input = layers.findIndex(item => item.renderKind === 'linkage' && /input|crank/i.test(item.label));
    const output = layers.findIndex(item => item.renderKind === 'linkage' && /output|rocker/i.test(item.label));
    const coupler = layers.findIndex(item => item.renderKind === 'linkage' && /coupler/i.test(item.label));
...
const STARTER_IMAGE_TEMPLATES: StarterImageTemplate[] = [
    { id: 'girl', label: 'Girl', fileName: 'girl.png', url: girlStarterUrl, thumbUrl: girlStarterThumbUrl },
    { id: 'boy', label: 'Boy', fileName: 'boy.PNG', url: boyStarterUrl, thumbUrl: boyStarterThumbUrl }
];
...
const App: React.FC = () => {
    ...
    const loadStarterImage = async (template: StarterImageTemplate) => {
        ...
        try {
            ...
            dispatch({ type: 'set_processing', processing: { stage: 'error', message: 'Starter failed', progress: 0, error: message } });
...
const MechanismFoundry = ({ project, foundry, setFoundry, selectedPart, selectedPath, goStage, onExport }: { project: ProjectState; foundry: FoundryState; setFoundry: (m: FoundryState) => void; selectedPart?: BodyPartLayer; selectedPath?: ProjectMotionPath; goStage: (stage: AppStage) => void; onExport: (pkg: FoundryExportPackage) => void }) => {
    ...
    const foundryRenderedLayerZ = useMemo(() =>
        foundryRenderedLayerZForMechanism(landedFoundry.type, foundryRenderPlan.layers, foundryStackLayerZ),
    ...
    const foundryOverlayZForHandle = (handleId?: string) => {
        ...
        if (landedFoundry.type === '4bar' && (handleId === 'A' || handleId === 'D')) return pin?.bottomZ ?? foundryOverlayZ;
        ...
        layout={{
            ...
            >
                <svg data-testid="foundry-preview-overlay" viewBox={`0 0 ${foundryProjectionSize.width} ${foundryProjectionSize.height}`} className="foundry-preview-overlay" aria-label="Foundry physical joint overlay" data-projection-aspect={(foundryProjectionSize.width / Math.max(1, foundryProjectionSize.height)).toFixed(3)}>
                    ...
                    {foundryParamHandles.length > 0 && <g data-testid="foundry-param-handles" data-handle-contract="4bar-A-B-C-D" data-projection="three-camera" data-handle-z-contract="board-pivots-bottom-floating-top" data-handle-z-map={foundryParamHandleZSummary}>
...
const MechanismParametricEditor = ({ mechanism, onChange, testId }: { mechanism: MechanismConfig; onChange: (updates: Partial<MechanismConfig>) => void; testId?: string }) => {
    ...
    const endpointGearOptions = mechanism.type === 'gear_linkage' ? FABRICATION_GEAR_SPECS.filter(spec => spec.attachmentHoleCentersMm.length > 0) : FABRICATION_GEAR_SPECS;
    ...
    return <div className="inspector-control-card compact-parametric-editor" data-testid={testId ?? 'mechanism-parametric-editor'}>
        {renderGearControls && <div className="space-y-2">
            ...
            {radii.map((radius, index) => {
                ...
                const options = mechanism.type === 'gear_linkage' && (index === 0 || isOutput) ? endpointGearOptions : FABRICATION_GEAR_SPECS;
        ...
        {renderLinkageControls && <div className="mt-3 space-y-2">
            ...
            {mechanism.type === 'gear_linkage' && <label className="block text-xs font-black uppercase tracking-wider text-slate-500">
                ...
                <select aria-label="Paired link length" className="field mt-1" value={linkageCellsForSceneLength(mechanism.couplerLength)} onChange={event => onChange({ couplerLength: linkageSceneLengthForCells(Number(event.target.value)) })}>
                    {FABRICATION_LINKAGE_SPECS.map(spec => <option key={spec.key} value={spec.cells}>{spec.cells}-cell</option>)}
                </select>
            </label>}
        </div>}
...
const foundryPinStacks = (
    ...
    options: { includeSpacerZ?: boolean; spacerZForPin?: (pin: FoundryPinStackPoint) => number[] | undefined } = {}
): FoundryPinStack[] => pinPoints.map(pin => {
    const localSpacerZ = options.includeSpacerZ ? options.spacerZForPin?.(pin) : undefined;
    const zIndexes = options.includeSpacerZ
        ? (localSpacerZ?.length ? pin.movingLayerIndexes : [...pin.movingLayerIndexes, ...pin.spacerLayerIndexes])
        : pin.movingLayerIndexes;
    const stackZ = [
        ...zIndexes.map(index => renderedLayerZ[index]).filter((z): z is number => typeof z === 'number'),
        ...(localSpacerZ ?? [])
    ];
    const minZ = stackZ.length ? Math.min(...stackZ) : renderedLayerZ[0] ?? FABRICATION_RENDER_LAYER_Z_STEP;
...
const ThreeFoundryPreview = ({ mechanism, simulation, kit, camera, rigOpacity, color, pathPoints, pathTraces, showGrid, showPathPreview, showTrail, showForces, showVelocity, explode, physicsRule, velocityMagnitude, forceMagnitude, frictionCoefficient, frictionMagnitude, constraintError, cameraLabel, isPickingAnchor, isOrbiting, isZooming, isPanning, onAnchorPick, onPointerDown, onPointerMove, onPointerUp, onPointerCancel, onWheel, onProjectionSizeChange, children }: ThreeFoundryPreviewProps) => {
    ...
    const assemblyPinPoints = useMemo(() => isGearTrain
        ? (mechanism.type === 'gear_linkage'
            ? [...gearCenters, simulation.state.j1, simulation.state.j2, simulation.state.effector].filter((point): point is Point => Boolean(point))
            : gearCenters)
    ...
    const boardPivotSpacerZ = (pin: FoundryPinStackPoint) => localSpacerZForPin(pin);
    ...
    const gearBoardSpacerSummary = isGearTrain
        ? pinStackPoints.slice(0, gearCenters.length).map(pin => `${pin.id}:${localSpacerZForPin(pin)?.toFixed(2) ?? 'n/a'}`).join(',')
        : '';
    const gearAxleZOrderSummary = isGearTrain
        ? pinStackPoints.slice(0, gearCenters.length).map(pin => {
            const gearLayerIndex = pin.movingLayerIndexes.find(index => renderPlan.layers[index]?.renderKind === 'gear');
            const gearZ = typeof gearLayerIndex === 'number' ? renderedLayerZ[gearLayerIndex] : undefined;
    ...
    >
        ...
        <div
            ...
            data-three-fourbar-ground-link-plane={mechanism.type === '4bar' ? 'A-D-ground-links-coplanar' : 'not-4bar'}
...
const MechanismLinkagePreview = ({ mechanism, simulation, kit, testId, compact = false }: { mechanism: MechanismConfig; simulation: ReturnType<typeof fitMechanismSimulation>; kit: PhysicalKitSettings; testId: string; compact?: boolean }) => {
    ...
    const links = (() => {
        ...
        if (mechanism.type === 'gear_linkage') return [
            link(s.j1, s.effector, 'drive-l4-linkage', 'mechanism-driver', 'driver'),
            link(s.j2, s.effector, 'output-l4-linkage', 'mechanism-output', 'output'),
